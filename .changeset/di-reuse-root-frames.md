...
Paired A/B against the previous build, six passes alternating which side ran first: `constant-resolve` **1.90×**, `container-level-activation-hook` **1.71×**, `realistic-graph-resolve-root` **1.30×**, `fan-out-tree-depth-3-breadth-4` **1.25×**, `scale-deep-transient-chain-512` **1.16×**, `to-alias-redirect` 1.10×, `singleton-class-1-dep` 1.09×.

`transient-class-1-dep` reads **0.93×**, negative in all six passes, and the mechanism is the same one that wins the other rows: a fresh array is in new space, so pushing a frame onto it needs no write barrier, while the shared pair is in old space and every push pays one. That row pushes a frame and does nothing else, so it is the one shape where the barrier costs more than the two allocations saved. Kept because it is one row at −7% against five between +16% and +90%.
