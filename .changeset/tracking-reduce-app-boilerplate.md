...
- `createCookieAnonymousId(options)` (`@codefast/tracking/client`) — an opt-in `document.cookie`-backed anonymous id resolver/clear pair for apps that don't need a custom identity strategy.
