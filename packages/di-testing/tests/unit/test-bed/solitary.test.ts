...
describe("TestBed.solitary", () => {
  it("auto-mocks every constructor dependency (zero-dep backend)", () => {
    ...
    expect(mocks.get(PaymentGatewayToken).charge.mock.calls.at(0)).toEqual(["u1", 42]);
    expect(mocks.get(EmailServiceToken).send.mock.calls.at(0)).toEqual(["alice@example.com", "Order confirmed — 42"]);
  });
  ...
  it("works with a Vitest mock factory and its matchers", () => {
    ...
    expect(mocks.get(PaymentGatewayToken).charge).toHaveBeenCalledWith("u1", 7);
    expect(mocks.get(EmailServiceToken).send).toHaveBeenCalledWith("alice@example.com", "Order confirmed — 7");
  });

  it("shares one mock across duplicate tokens", () => {
    const { unit, mocks } = TestBed.solitary(DoubleLogger).compile();

    expect(unit.first).toBe(unit.second);
    expect(unit.first).toBe(mocks.get(LoggerToken));
  });

  it("resolves a named dependency", () => {
    const { unit, mocks } = TestBed.solitary(NamedConsumer).compile();

    expect(unit.logger).toBe(mocks.get(LoggerToken));
  });

  it("resolves a tagged dependency", () => {
    const { unit, mocks } = TestBed.solitary(TaggedConsumer).compile();

    expect(unit.logger).toBe(mocks.get(LoggerToken));
  });

  it("binds every tagged slot even when tag values stringify identically", () => {
    const { unit, mocks } = TestBed.solitary(CollidingTagConsumer).compile();

    expect(unit.first).toBe(mocks.get(LoggerToken));
    expect(unit.second).toBe(mocks.get(LoggerToken));
  });

  it("resolves an accessor-injected dependency", () => {
    const { unit, mocks } = TestBed.solitary(AccessorConsumer).compile();
    unit.notify("bob@example.com");

    expect(mocks.get(EmailServiceToken).send.mock.calls.at(0)).toEqual(["bob@example.com", "hello"]);
  });

  it("mocks optional and multi dependencies", () => {
    const { unit, mocks } = TestBed.solitary(ReportService).compile();

    expect(unit.logger).toBe(mocks.get(LoggerToken));
    expect(unit.plugins).toEqual([mocks.get(PluginToken)]);
  ...
  it("compiles an accessor-only class that has no @injectable", () => {
    const { unit, mocks } = TestBed.solitary(AccessorOnlyService).compile();
    unit.notify("vip@example.com");

    expect(mocks.get(EmailServiceToken).send.mock.calls.at(0)).toEqual(["vip@example.com", "accessor-only"]);
  });
  ...
  it("builds the unit through compileAsync", async () => {
    const { unit, mocks } = await TestBed.solitary(OrderProcessor)
      .mock(UserServiceToken)
      .stub((fn) => ({ findUser: fn().mockReturnValue({ id: "u1", email: "alice@example.com" }) }))
      .compileAsync();
    ...
    expect(mocks.get(PaymentGatewayToken).charge.mock.calls.at(0)).toEqual(["u1", 9]);
  });
});
