...
import { DateAfter, DateBefore, DateInterval, DateRange, DayOfWeek } from '../types';
...
describe('when matching date interval (open)', () => {
  ...
  test('should return true for the days before', () => {
    const result = dateMatchModifiers(subDays(testDay, 8), [matcher], defaultDateLib);
    expect(result).toBe(true);
  });
  test('should return true for the days after', () => {
    const result = dateMatchModifiers(addDays(testDay, 8), [matcher], defaultDateLib);
    expect(result).toBe(true);
  });
});
