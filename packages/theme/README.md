...
`react` and `react-dom` 19 or later are peer dependencies, and `@codefast/theme` requires Node.js 24 or later, Chrome
and Edge 136, Firefox 136, or Safari 18.4 or later in the browser ([support policy](../../SUPPORT.md#browsers)), and
TypeScript 7 or later for its types. The package is published on 0.x and versioned on its own track: breaking changes
ship as minor versions, so pin the minor version when you need stability.
