...
export class CliPortTelemetryService implements CliPortTelemetryPort {
    ...
    return new Proxy(implementation, {
      get: (target, propertyKey, receiver) => {
        ...
        return (...callArgs: unknown[]) => {
          const argSummary = callArgs.map((arg) => this.summarizeTelemetryArg(arg)).join(", ");
  ...
  withOptionalPortTelemetry<T extends object>(
    ...
    return this.withCliPortTelemetry({ portName, implementation, logger });
  }

  private summarizeTelemetryArg(value: unknown, maxLen = 160): string {
