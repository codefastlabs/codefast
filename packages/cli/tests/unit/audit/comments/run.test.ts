...
describe("runCommentAudit", () => {
  ...
  it("enforces the divider in an ignore file while leaving its content rules off", () => {
    // A legacy hash divider plus a line that would trip the doc-pointer rule if this were code.
    const gitignore = ["# === Dependencies ===", "node_modules", "# see ARCHITECTURE.md for the layout"].join("\n");
    const { fs, contents } = createSourceTestFilesystem({ [path.join(rootDir, ".gitignore")]: gitignore });
    ...
    expect(outcome.value).toMatchObject({ breakageCount: 0, dividerCount: 1, fixedCount: 1 });
    const fixed = contents.get(path.normalize(path.join(rootDir, ".gitignore")));
    expect(fixed).toContain("# ── Dependencies ──");
    // The doc pointer survives untouched — ignore files carry only the divider convention.
    expect(fixed).toContain("# see ARCHITECTURE.md for the layout");
  });
