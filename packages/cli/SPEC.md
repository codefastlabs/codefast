...
1. [Bối cảnh và mục tiêu](#1-bối-cảnh-và-mục-tiêu)
   - [1.1 Vấn đề với cấu trúc hiện tại (`src/lib/**`)](#11-vấn-đề-với-cấu-trúc-hiện-tại-srclib)
   - [1.2 Mục tiêu khi viết lại](#12-mục-tiêu-khi-viết-lại)
...
2. [Nền tảng kỹ thuật và ràng buộc monorepo](#2-nền-tảng-kỹ-thuật-và-ràng-buộc-monorepo)
   ...
   - [2.2 Ranh giới import (kiểm chứng tự động)](#22-ranh-giới-import-kiểm-chứng-tự-động)
...
6. [Quy ước `@codefast/di`](#6-quy-ước-codefastdi)
   ...
   - [6.3 Bind port → bộ thích ứng; cross-domain (`config` → miền khác)](#63-bind-port--bộ-thích-ứng-cross-domain-config--miền-khác)
...
9. [Kế hoạch chuyển đổi](#9-kế-hoạch-chuyển-đổi)
   ...
   - [Bước 0 — Chốt mục 10 + ranh giới import](#bước-0--chốt-mục-10--ranh-giới-import)
...
3. **Đồ thị DI một nghĩa cho lệnh** — snapshot cũ dễ tạo **hai singleton** cho cùng một `*Command` vì bind **thừa** class token (`bind(ArrangeCommand).toSelf()…`) **và** bind `CliCommandToken`. Thiết kế mới: **chỉ** `bind(CliCommandToken).to(*Command).whenNamed(…).singleton()` trong từng miền — **không** bind class token cho lệnh; `bootstrap` lấy danh sách bằng **`resolveAll(CliCommandToken)`** ([mục 6.4](#64-đăng-ký-nhiều-clicommand-multi-binding--đã-hỗ-trợ-sẵn)). **`toAlias`** không dùng cho đăng ký lệnh CLI. Đây là thay đổi **cố ý** so với composition-root cũ.
...
### 2.2 Ranh giới import (kiểm chứng tự động)

Quy tắc [mục 4.7](#47-ánh-xạ-vào-cây-thư-mục-trong-gói) **phải** được kiểm chứng bằng công cụ trong CI, không chỉ qua review.

- **Chốt:** dùng **`dependency-cruiser`** (hoặc tương đương, ví dụ `eslint-plugin-boundaries`) với rule phản ánh tầng trong từng `domains/<miền>/`, **cấm** import lớp cụ thể giữa hai miền (`domains/foo` → `domains/bar/**`), và **danh sách cho phép** tới `shell/contracts/**` (xem [4.4](#44-thành-phần-component-và-gói-theo-miền)).
...
- **Hạn chót:** cấu hình tối thiểu (file rule + lệnh trong `package.json` của gói + chạy trong CI) phải có **trước khi merge** nhánh đầu tiên chứa bất kỳ `src/domains/**` nào — cùng cửa sổ với [bước 0](#bước-0--chốt-mục-10--ranh-giới-import) / PR dựng khung cây. **Không** trì hoãn tới “khi đã có nhiều mã”.
...
module.exports = {
  forbidden: [
    ...
    // EA trong từng miền: domain/ không import application/ (đảo ngược [4.7](#47-ánh-xạ-vào-cây-thư-mục-trong-gói))
    {
      name: "no-domain-imports-application",
      comment: "SPEC §4.7 — domain không phụ thuộc application",
      severity: "error",
...
- **`null` vs bỏ trường:** với các trường đã nêu tường minh (ví dụ `hookError: string | null` — [3.8](#38-móc-vòng-đời-onafterwrite)), payload phải có **trường đó** với giá trị `null` hoặc chuỗi — **không** “lặn” trường khi đặc tả đã chốt nullable.
- **Fixture / test gốc (đường dẫn cụ thể — sau [9.1](#91-lưu-trữ-mã-và-kiểm-thử-cũ-trong-old) nằm dưới `old/tests/`):** dùng làm oracle JSON và hành vi lệnh khi port — ví dụ:
...
- **`ScopeViolationError`:** từ `validate()` khi `NODE_ENV !== "production"` — **không** ánh xạ qua `consumeCliAppError` hay mô hình `AppError`; xử lý như **lỗi hạ tầng / wiring**, thoát **`1`** (`CLI_EXIT_GENERAL_ERROR`, [mục 3.3](#33-mã-thoát-và-lớp-lỗi-thống-nhất)), in lỗi chẩn đoán (stderr). **`finally`** luôn gọi `dispose()` khi container đã tạo — kể cả sau `validate()` ném.
- **`createCliRuntimeContainer`:** định nghĩa trong `bootstrap/cli-composition-root.ts` — chỉ wiring DI ([mục 7.3](#73-cây-src-đầy-đủ-và-chỗ-đặt-module-di)).
- **Production** bỏ `validate()` / `await initializeAsync()` để giảm chi phí — **hệ quả:** `@postConstruct` **không** chạy trong production. **Bắt buộc:** không dùng `@postConstruct` cho trạng thái thiết yếu suốt process; chỉ tối ưu tùy chọn (xem [mục 2.1](#21-gói-codefastdi--api-dùng-cho-cli)). **Bù trừ:** test/CI với `NODE_ENV !== "production"` — [mục 8](#8-chiến-lược-kiểm-thử).
...
- Các bước còn lại: resolve danh sách lệnh — chuẩn **`container.resolveAll(CliCommandToken)`** (sau `load` module — [mục 6.4](#64-đăng-ký-nhiều-clicommand-multi-binding--đã-hỗ-trợ-sẵn)), rồi tạo program gốc (`configureHelp`, `showHelpAfterError`, `--no-color`), `await program.parseAsync(argv, { from: "node" })`.

### 3.2 Tùy chọn toàn cục và quy ước nhập–xuất

- **Tùy chọn gốc**: `--no-color` (`Commander` ánh xạ tới `color: false` khi phân tích cú pháp toàn cục — `mirror` dùng `parseGlobalCliOptions` từ [mục 7.4](#74-bố-cục-shell) và `command.optsWithGlobals()`).
- **Phiên bản / trợ giúp**: `-V` / `--version`, `-h` / `--help` (có sẵn trên lệnh gốc). **Parity chuỗi:** không yêu cầu help **byte-for-byte** giống `old/`; **bắt buộc** có integration test đối chiếu với `old/tests/integration/core/bin.integration.test.ts` (hoặc `tests/integration/core/...` trước 9.1) sao cho **cùng tập** lệnh gốc / lệnh con xuất hiện trong output (tên và hierarchy), và `--version` in **đúng** trường `"version"` từ `package.json` của gói ([3.1](#31-điểm-vào-chương-trình-commander-vòng-đời-container)).
...
**Cảnh báo**: `LoadCodefastConfigUseCase` báo cảnh báo qua `ConfigWarningReporterPort` (in stdout với tiền tố màu vàng). Port này nằm trong `domains/config/application/ports/`; adapter **`config-warning-reporter.adapter.ts`** trong `domains/config/infrastructure/adapters/`, `@injectable([inject(CliLoggerToken), …])` (hoặc token logger đã chốt trong `shell/contracts/tokens.ts`) — **không** cần `presentation/`; binding trong `config.module.ts` — [mục 6.3](#63-bind-port--bộ-thích-ứng-cross-domain-config--miền-khác).
...
**Ranh giới lớp (`ConfigWarningReporterAdapter`):** màu vàng / tiền tố dòng cảnh báo là **hiện thực** của `ConfigWarningReporterPort` trong `domains/config/infrastructure/` — đây là bộ thích ứng thứ hai theo port, **không** “tầng presentation” cho miền `config` (miền này không có `presentation/`; [4.7](#47-ánh-xạ-vào-cây-thư-mục-trong-gói)). Logic **khi nào** cảnh báo (sau parse Zod, loại cảnh báo…) thuộc use case / domain; adapter chỉ định dạng IO theo hợp đồng port.
...
- Cờ: `-v`/`--verbose`, `--json`.
- Dùng `parseGlobalCliOptions` ([mục 7.4](#74-bố-cục-shell)) với `optsWithGlobals()` cho màu. Kết hợp **`--verbose` + `--json`:** tuân [mục 3.2](#32-tùy-chọn-toàn-cục-và-quy-ước-nhập-xuất) — không in verbose hướng người khi `--json`.
...
- Người nghe tiến trình tắt khi `--json`. Kết hợp **`--verbose` + `--json`** (nếu lệnh có verbose): tuân [mục 3.2](#32-tùy-chọn-toàn-cục-và-quy-ước-nhập-xuất).
...
- **`LoadCodefastConfigUseCase`**: tải + cảnh báo; dùng chung cho bước mở đầu các lệnh — **cơ chế DI** với miền khác: [mục 6.3](#63-bind-port--bộ-thích-ứng-cross-domain-config--miền-khác).
- **`parseGlobalCliOptions`**: ánh xạ tùy chọn toàn cục Commander (ví dụ `--no-color`) — đặt trong `shell/contracts/` ([mục 7.4](#74-bố-cục-shell)), không thuộc `domains/config`.
- **Duyệt / sửa mã nguồn TypeScript** dùng chung (AST, text edit…) giữa `arrange` và `tag`: **chưa có** đường dẫn thư mục cố định trong cây `src/` cho đến khi [mục 10](#10-quyết-định-còn-mở) được chốt — không dùng tên `shared/source-code` như đã tồn tại.
...
2. **Composition root / DI:** `createCliRuntimeContainer()`, `validate()`, `await initializeAsync()` thành công trong môi trường test với `NODE_ENV !== "production"` ([mục 3.1](#31-điểm-vào-chương-trình-commander-vòng-đời-container), [mục 8](#8-chiến-lược-kiểm-thử)).
...
7. **Lỗi wiring DI — `ScopeViolationError`:** với `NODE_ENV !== "production"`, kịch bản (test hoặc fixture) làm `validate()` thất bại → thoát `1`, stderr có chẩn đoán; **không** đi qua `AppError` ([3.1](#31-điểm-vào-chương-trình-commander-vòng-đời-container), [3.3](#33-mã-thoát-và-lớp-lỗi-thống-nhất)).
8. **Cảnh báo schema config:** fixture `codefast.config.*` có khóa thừa / cảnh báo tiêu biểu — sau prelude, xác nhận đường cảnh báo đi qua `ConfigWarningReporterPort` (màu / tiền tố theo [3.4](#34-cấu-hình-codefastconfig)) và lệnh vẫn tiếp tục khi chỉ là cảnh báo.
9. **Lỗi móc `onAfterWrite`:** `arrange apply` hoặc `tag` với móc ném/.reject — payload JSON có `hookError` **chuỗi**, `ok: false`, thoát `1`; so oracle [đầu mục 3](#3-hành-vi-sản-phẩm--tương-đương-bắt-buộc) và [3.8](#38-móc-vòng-đời-onafterwrite).
10. **`--verbose` + `--json`:** trên lệnh có verbose (`mirror`, `tag` nếu có), xác nhận **không** có log verbose hướng người trên stdout khi `--json`; stdout chỉ một object JSON ([3.2](#32-tùy-chọn-toàn-cục-và-quy-ước-nhập-xuất)).
11. **Đường lỗi validation CLI (`AppError` `VALIDATION_ERROR`):** argv / schema Zod — thoát `2`, thông điệp qua `consumeCliAppError` / presenter (ít nhất một lệnh; ví dụ thiếu đối số bắt buộc).
...
- **Ví dụ chốt:** prelude tải cấu hình — interface + token trong `shell/contracts/`, implementation trong `domains/config/`, consumer inject token chỉ từ `shell/contracts` ([mục 6.3](#63-bind-port--bộ-thích-ứng-cross-domain-config--miền-khác)).
...
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| Port (hợp đồng ứng dụng)                                | `*.port.ts` — **trừ** port xuyên miền trong `shell/contracts/*.port.ts` ([7.4](#74-bố-cục-shell))                                                 | `file-writer.port.ts`                                       |
...
| Tiện ích thuần trong `shell/contracts/`                 | **`kebab-case.ts`**, export hàm/kiểu nhỏ — **không** dùng suffix `*.port.ts` / `*.use-case.ts` ([7.4](#74-bố-cục-shell))                          | `parse-global-cli-options.ts`                               |

**Lưu ý:** không dùng `*.interface.ts` làm tên chuẩn; port là `*.port.ts`. Lớp trùng tên với port có thể đặt cùng stem (ví dụ `file-writer.port.ts` export `FileWriterPort`). **Token DI:** tệp **`*.tokens.ts`** trong **cùng thư mục `application/`** của miền đó, gom token gắn với các port/use case của miền; token và hợp đồng xuyên miền nằm trong `shell/contracts/` ([mục 7.4](#74-bố-cục-shell)). **Kiểu `*Result` / payload JSON:** nằm trong `domain/` (`types.domain.ts` hoặc `*.domain.ts`) trừ khi là DTO **chỉ** phục vụ một use case và cần tách — khi đó vẫn **dưới** `application/` hoặc `domain/` của cùng miền, **không** đặt dưới `shell/`; ghi trong PR nếu ngoại lệ.
...
- Bind thêm ở gốc glue chỉ khi **không** thuộc một module đơn (hiếm); **không** nhân đôi đăng ký `CliCommandToken` nếu đã bind trong từng `{miền}.module.ts`. Sau `load`, danh sách lệnh lấy bằng `container.resolveAll(CliCommandToken)` ([mục 6.4](#64-đăng-ký-nhiều-clicommand-multi-binding--đã-hỗ-trợ-sẵn)) — không bắt buộc hàm helper một dòng.
...
Miền **không** `import` lớp cụ thể từ `domains/config/**` ([4.4](#44-thành-phần-component-và-gói-theo-miền)). Thay vào đó:
...
5. **`bootstrap` / thứ tự `load`:** `ConfigModule` được `load` **trước** các module cần resolve `LoadCodefastConfigToken` ([6.2](#62-gốc-composition-bootstrap)).
...
// ConfigWarningReporterAdapter: @injectable([inject(CliLoggerToken), …]) — stdout, tiền tố vàng theo [3.4](#34-cấu-hình-codefastconfig)
```

**Khớp tên:** adapter đặt tại `domains/config/infrastructure/adapters/config-warning-reporter.adapter.ts` (theo cây [7.3](#73-cây-src-đầy-đủ-và-chỗ-đặt-module-di)).
...
**Đối chiếu snapshot:** trong `old/` có thể thấy `load(ArrangePresentationModule, ArrangeModule)` và glue resolve theo class — **`src/` mới** chỉ `load(ArrangeModule)` và **một** chiến lược token ([6.1](#61-module-theo-miền)).
...
2. **Trục thứ hai — Explicit Architecture trong từng lát cắt:** trong `domains/<miền>/` luôn có **`domain/`**, **`application/`**, **`infrastructure/`** — ánh xạ [mục 4.2–4.3](#42-port-và-bộ-thích-ứng-lục-giác--vành) và bảng [4.7](#47-ánh-xạ-vào-cây-thư-mục-trong-gói). **`presentation/`** chỉ tồn tại khi miền có **biên điều khiển CLI** (`*Command`, Commander). Miền **không** có lệnh riêng (ví dụ **`config`**) **không** tạo `presentation/` rỗng — tránh artifact thừa.
...
Chiều phụ thuộc: khi có `presentation/`, thì `presentation` → `application` → `domain`; `infrastructure` hiện thực port mà `application` khai báo — [mục 4.7](#47-ánh-xạ-vào-cây-thư-mục-trong-gói).

### 7.3 Cây src đầy đủ và chỗ đặt module DI

- **`bin.ts`:** điểm vào process — ủy quyền cho `bootstrap/run-cli.ts`.
- **`bootstrap/run-cli.ts`:** vòng đời **process** — `try` / `catch` / `finally` theo [mục 3.1](#31-điểm-vào-chương-trình-commander-vòng-đời-container) (`createCliRuntimeContainer`, `validate`/`initializeAsync` khi không production, `ScopeViolationError` → thoát `1`, `dispose` trong `finally`); dựng `Commander`, `parseAsync`. **Không** chứa `Module.create` / bind chi tiết.
- **`bootstrap/cli-composition-root.ts`:** **wiring DI thuần túy** — export `createCliRuntimeContainer()` (`Container.create`, `load(ConfigModule, …)`, bind glue không thuộc module theo [mục 6](#6-quy-ước-codefastdi)). Không gọi `program.parse`.
...
Các thư mục con dưới đây là **quy ước gói** để nhìn đường dẫn là biết vai trò EA; tệp vẫn tuân [mục 4.8](#48-quy-ước-đặt-tên-tệp) (hậu tố `.use-case.ts`, `.port.ts`, …). **`presentation/`:** bỏ qua bảng này nếu miền không có CLI.
...
- **`shell/contracts/`:** port, token và kiểu dùng chung giữa miền (`CliCommand`, …). Giữ **mỏng** — **chỉ** hợp đồng + token + hàm thuần không mang logic nghiệp vụ dài; mọi ca nghiệp vụ (kể cả tải cấu hình) nằm dưới `domains/`, không dưới `shell/`. **Quy tắc cứng** (domain `if`/`switch`…): [mục 4.4](#44-thành-phần-component-và-gói-theo-miền).
- **`load-codefast-config.port.ts`:** `LoadCodefastConfigPort` + `LoadCodefastConfigToken` — consumer các miền khác chỉ import từ đây ([6.3](#63-bind-port--bộ-thích-ứng-cross-domain-config--miền-khác)).
- **`parse-global-cli-options.ts`:** tệp theo **`kebab-case.ts`** ([mục 4.8](#48-quy-ước-đặt-tên-tệp) — tiện ích `shell/contracts/`). Export **`parseGlobalCliOptions`**, nhận kiểu tương thích `optsWithGlobals()` / `Commander` (theo [3.2](#32-tùy-chọn-toàn-cục-và-quy-ước-nhập-xuất)), trả ví dụ `{ noColor: boolean }`. **Không** đặt trong `domains/config` (không thuộc bounded context file `codefast.config`).
...
- **Fail-fast DI (bù production bỏ `validate`):** ít nhất **một** test tích hợp (hoặc file smoke chạy trong CI cho gói) tạo `createCliRuntimeContainer()` với `NODE_ENV` **khác** `"production"`, gọi `validate()` và `await initializeAsync()` thành công — bắt sai ràng buộc binding trước khi merge ([3.1](#31-điểm-vào-chương-trình-commander-vòng-đời-container)).
- **Ranh giới kiến trúc:** cấu hình [mục 2.2](#22-ranh-giới-import-kiểm-chứng-tự-động) chạy trong CI (cùng job hoặc gate tương đương); vi phạm = không merge.
...
- **Xoá `old/`:** khi **DoD toàn gói** đạt ([9.2](#92-hoàn-thành-theo-miền-definition-of-done) cho **`config`**, `mirror`, `tag`, `arrange` và [mục 8](#8-chiến-lược-kiểm-thử) — độ phủ ≥ 80% dòng trên `src/`, parity [mục 3](#3-hành-vi-sản-phẩm--tương-đương-bắt-buộc)), **xoá thư mục `old/`** khỏi working tree của gói. Cần giữ snapshot lịch sử → dùng **git tag** hoặc nhánh riêng; **không** duy trì `old/` song song như nguồn sự thật thứ hai sau thời điểm đó.

### Bước 0 — Chốt mục 10 + ranh giới import

Sau **9.1** (nếu dùng snapshot), và **trước mọi dòng mã** của cây `src/` mới:

**Bắt buộc** giải quyết toàn bộ [mục 10](#10-quyết-định-còn-mở), vì các quyết định đó ảnh hưởng `bootstrap/` và cách chia mã dùng chung (AST). Đăng ký `CliCommand` đã được chốt theo khả năng thực tế của `@codefast/di` tại [mục 6.4](#64-đăng-ký-nhiều-clicommand-multi-binding--đã-hỗ-trợ-sẵn). **Đồng thời** có cấu hình tối thiểu [mục 2.2](#22-ranh-giới-import-kiểm-chứng-tự-động) (dependency-cruiser hoặc tương đương) chạy được trên CI — không merge `src/domains/**` trước đó. Không có bước 0, implementer sẽ phải đoán và có nguy cơ refactor lớn.

### 9.2 Hoàn thành theo miền (Definition of Done)

**`config` (nền — phải đạt trước các miền còn lại):** coi **xong** khi **đồng thời**:

1. [mục 3.4](#34-cấu-hình-codefastconfig) (tìm file, tải, schema, cảnh báo) tương đương; prelude dùng được từ đường đi chuẩn của các lệnh.
2. **`LoadCodefastConfigPort` / token** trong `shell/contracts` + bind trong `config.module.ts` + consumer chỉ phụ thuộc shell ([6.3](#63-bind-port--bộ-thích-ứng-cross-domain-config--miền-khác)); không vi phạm [4.4](#44-thành-phần-component-và-gói-theo-miền).
...
3. Độ phủ dòng trên mã `src/` của miền đó **≥ 80%** (hoặc ngoại lệ có lý do ghi trong PR); với `arrange`, thêm mục tiêu branch như [mục 8](#8-chiến-lược-kiểm-thử).
...
2. Dựng khung `bootstrap` + `shell/contracts` + **`domains/config` (đạt DoD [9.2](#92-hoàn-thành-theo-miền-definition-of-done) `config`)** + một miền mẫu có lệnh (nên **`mirror`** — ít lớp hiển thị hướng người nhất).
3. Hoàn tất **DoD** [9.2](#92-hoàn-thành-theo-miền-definition-of-done) theo thứ tự **`mirror` → `tag` → `arrange`** (sau khi `config` xong).
...
**Hạn:** phải chốt **hết** các mục dưới đây tại [bước 0](#bước-0--chốt-mục-10--ranh-giới-import) (sau [9.1](#91-lưu-trữ-mã-và-kiểm-thử-cũ-trong-old)). Mỗi mục **phải** kết thúc bằng một trong hai phương án đã chọn + **đường dẫn / quy ước** ghi trong đặc tả (mục 7 / 4.8) hoặc trong PR merge quyết định — không để “chọn sau” vô hạn.

**Trách nhiệm & enforcement (bắt buộc trước code `src/` mới):**

| Mục trong §10                          | Owner (vai trò)                                                                                                                                       | Deadline / milestone                                                                          |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| AST / mã nguồn chung `arrange` ↔ `tag` | Maintainer gói (ghi **tên hoặc @handle** trong PR bước 0 — không merge khi ô Owner trống); nếu SPEC public không ghi tên cố định, PR là nguồn sự thật | [Bước 0](#bước-0--chốt-mục-10--ranh-giới-import) — cùng PR hoặc PR ngay trước PR khung `src/` |
| Chính sách `index.ts` barrel           | Cùng owner trên                                                                                                                                       | Bước 0                                                                                        |

- **Ai quyết định:** owner cột hai (hoặc delegate ghi rõ trong PR) — không để implementer tự chốt ngầm. **PR [bước 0](#bước-0--chốt-mục-10--ranh-giới-import):** phải có dòng `Owner: …` trong mô tả (hoặc checklist dưới đây) cho **từng** hàng quyết định §10; reviewer **không approve** nếu trống.
- **Checklist PR (dán vào mô tả PR khi merge quyết định §10):** `[ ] Mục 10 — AST: phương án A/B đã chọn; SPEC §3.9 / §7.3 cập nhật` · `[ ] Mục 10 — barrel: chính sách đã chọn; CONTRIBUTING hoặc §4.8 cập nhật nếu cần` · `[ ] Owner + ngày chốt ghi trong PR hoặc commit message`.

**Ghi nhận quyết định (ADR nhẹ):** mỗi PR chốt mục 10 nên có hai câu ngắn: _Lý do chọn:_ … ; _Phương án đã xét và loại:_ … — không bắt buộc thư mục `docs/adr/` riêng.

**Tiêu chí để chốt phương án “AST / mã nguồn dùng chung”:**

| Tiêu chí                  | Một module dùng chung dưới `shell/` (hoặc `shell/<tên đã chốt>/`)                | Nhân đôi tối thiểu trong `domains/arrange` + `domains/tag`                                         |
| ------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Coupling                  | Một nơi sửa — `arrange` và `tag` phụ thuộc cùng abstraction trong `shell`        | Hai cây độc lập — ít import chéo giữa miền, nhiều khả năng lệch hành vi nếu sửa một bên            |
| Trùng lặp / drift         | Ít duplicate mã duyệt TS; cần ranh giới rõ (chỉ port + tiện ích, không business) | Có thể duplicate collector/visitor; phải **cô lập** phần trùng (copy có chủ đích + test song song) |
| Phạm vi `shell/contracts` | Phải giữ **mỏng**: chỉ kiểu/port/token, không “core” phình to                    | `shell` gần như không đổi; chi phí trùng nằm ở từng miền                                           |

- **AST / bộ duyệt mã nguồn dùng chung (giữa `arrange` và `tag`):** chọn **một** phương án — (A) module/hợp đồng dưới `shell/` (ví dụ chỉ export port + kiểu thuần, implementation ở từng miền **hoặc** một gói con rõ tên) **hoặc** (B) nhân đôi có kiểm soát trong từng `domains/…`. **Cấm** cả hai song song không có ranh giới (trùng logic AST ở ba chỗ). Sau khi chốt, cập nhật [mục 3.9](#39-phần-dùng-chung-giữa-các-miền) và cây [mục 7.3](#73-cây-src-đầy-đủ-và-chỗ-đặt-module-di) cho khớp.

  **Khuyến nghị mặc định (khi chưa có lý do đặc biệt):** phần dùng chung **chỉ** là kiểu / port trừu tượng (không chứa visitor/collector thật) → ưu tiên **A** để một nơi sửa hợp đồng. Cần chia sẻ **implementation** duyệt AST (code dài, phụ thuộc TS API) → ưu tiên **B** (nhân đôi có kiểm soát + test song song) để **không phình** `shell/` thành “core” ẩn — thống nhất với [mục 4.4](#44-thành-phần-component-và-gói-theo-miền) và `shell` mỏng.

- **`index.ts` barrel (gom export):** **Khuyến nghị mặc định khi chốt:** **không** dùng `index.ts` barrel trong **`domains/<miền>/domain/`** và **`domains/<miền>/application/`** (tránh vòng phụ thuộc, khó đọc đồ thị import, bundler chậm). **Cho phép** tối đa **một** `index.ts` tại **gốc** `domains/<miền>/` nếu miền cần re-export **public surface** hẹp cho `shell/contracts` hoặc cho test — phải ghi rõ trong PR. Nếu đội chọn “cấm mọi barrel” hoặc “bắt buộc barrel theo tầng”, cập nhật một dòng chính sách vào hướng dẫn đóng góp gói sau khi [bước 0](#bước-0--chốt-mục-10--ranh-giới-import).

> **Đăng ký lệnh:** `@codefast/di` hỗ trợ multi-binding + `whenNamed` + `resolveAll` / `@injectable([injectAll(...)])` ([mục 6.4](#64-đăng-ký-nhiều-clicommand-multi-binding--đã-hỗ-trợ-sẵn)). Không có class `CliCommandRegistry` trong codebase. **`src/` mới** chỉ bind **`CliCommandToken` → `*Command`** (một dòng / lệnh, có `whenNamed`) — **cấm** thêm bind class token cho `*Command` như snapshot; không dùng `toAlias` cho lệnh. Sai lệch so với chốt này → nguy cơ hai instance hoặc last-wins (mất lệnh).
