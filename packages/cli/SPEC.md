...
**Phạm vi “đã chốt” vs “chốt trước code miền”:** toàn bộ tài liệu là **đặc tả làm việc**: (1) **hành vi** [mục 3](#3-product-behavior) và các quy tắc **EA / DI / cây `src/`** ở [mục 4](#4-explicit-architecture)–[7](#7-src-directory-layout) là chuẩn triển khai. **[Mục 10](#10-open-decisions)** chỉ chứa **hai chỗ còn phụ thuộc lựa chọn nhị phân** (chia sẻ AST; barrel) — chúng **bắt buộc chốt trước PR đầu tiên có `src/domains/**`** ([bước 0](#step-0-section-10-import-boundaries)), không làm mất hiệu lực mục 3 hay cách bố trí tầng EA. Đến khi mục 10 xong, implementer có **đường dẫn và ranh giới import** đầy đủ (kể cả `arrange`/`tag`).
...
9. [Kế hoạch chuyển đổi](#9-migration-plan)
   ...
   - [Bước 0 — Chốt mục 10 + ranh giới import](#step-0-section-10-import-boundaries)
   - [9.2 Hoàn thành theo miền (Definition of Done)](#9-2-definition-of-done)
10. [Quyết định còn mở](#10-open-decisions)
...
- **Duyệt / sửa mã nguồn TypeScript** dùng chung (AST, text edit…) giữa `arrange` và `tag`: **chưa có** đường dẫn thư mục cố định trong cây `src/` cho đến khi [mục 10](#10-open-decisions) được chốt — không dùng tên `shared/source-code` như đã tồn tại.
...
**Bắt buộc** giải quyết toàn bộ [mục 10](#10-open-decisions), vì các quyết định đó ảnh hưởng `bootstrap/` và cách chia mã dùng chung (AST). Đăng ký `CliCommand` đã được chốt theo khả năng thực tế của `@codefast/di` tại [mục 6.4](#6-4-multibind-clicommand). **Đồng thời** có cấu hình tối thiểu [mục 2.3](#2-3-import-boundary-enforcement) (dependency-cruiser hoặc tương đương) chạy được trên CI — không merge `src/domains/**` trước đó. Không có bước 0, implementer sẽ phải đoán và có nguy cơ refactor lớn.
...
**Phạm vi chặn merge:** hai quyết định trong mục này là **tiền đề cứng** trước PR **đầu tiên** có `src/domains/**` ([bước 0](#step-0-section-10-import-boundaries)). Phần còn của đặc tả (mục 3–9) **vẫn hiệu lực**; đừng nhầm “chờ chốt AST/barrel” với “toàn bộ SPEC chỉ là nháp”.

**Hạn:** phải chốt **hết** các mục tiểu mục dưới đây tại [bước 0](#step-0-section-10-import-boundaries) (sau [9.1](#9-1-old-snapshot)). Mỗi mục **phải** kết thúc bằng một trong hai phương án đã chọn + **đường dẫn / quy ước** ghi trong đặc tả (mục 7 / [4.7](#4-7-file-naming-conventions)) hoặc trong PR merge quyết định — không để “chọn sau” vô hạn.

**Trách nhiệm & enforcement (bắt buộc trước code `src/` mới):**

| Mục trong §10                          | Owner (vai trò)                                                                                                                                       | Deadline / milestone                                                                        |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| AST / mã nguồn chung `arrange` ↔ `tag` | Maintainer gói (ghi **tên hoặc @handle** trong PR bước 0 — không merge khi ô Owner trống); nếu SPEC public không ghi tên cố định, PR là nguồn sự thật | [Bước 0](#step-0-section-10-import-boundaries) — cùng PR hoặc PR ngay trước PR khung `src/` |
| Chính sách `index.ts` barrel           | Cùng owner trên                                                                                                                                       | Bước 0                                                                                      |

- **Ai quyết định:** owner cột hai (hoặc delegate ghi rõ trong PR) — không để implementer tự chốt ngầm. **PR [bước 0](#step-0-section-10-import-boundaries):** phải có dòng `Owner: …` trong mô tả (hoặc checklist dưới đây) cho **từng** hàng quyết định §10; reviewer **không approve** nếu trống.
- **Checklist PR (dán vào mô tả PR khi merge quyết định §10):** `[ ] Mục 10 — AST: phương án A/B đã chọn; SPEC §3.9 / §7.3 cập nhật` · `[ ] Mục 10 — barrel: chính sách đã chọn; CONTRIBUTING hoặc §4.7 cập nhật nếu cần` · `[ ] Owner + ngày chốt ghi trong PR hoặc commit message`.
