...
3. **Đồ thị DI một nghĩa cho lệnh** — snapshot cũ dễ tạo **hai singleton** cho cùng một `*Command` vì bind **thừa** class token (`bind(ArrangeCommand).toSelf()…`) **và** bind `CliCommandToken`. Thiết kế mới: **chỉ** `bind(CliCommandToken).to(*Command).whenNamed(…).singleton()` trong **`bootstrap/cli-application.module.ts`** — **không** bind class token cho lệnh; `bootstrap` lấy danh sách bằng **`resolveAll(CliCommandToken)`** ([mục 6.4](#6-4-multibind-clicommand)). **`toAlias`** không dùng cho đăng ký lệnh CLI. Đây là thay đổi **cố ý** so với composition-root cũ.
...
- `.json`: `readFile` + `JSON.parse` + `CodefastConfigSchemaPort.safeParseLoadedConfig`.
- `.js` / `.mjs` / `.cjs`: `jiti(jitiBaseDir, { interopDefault: true, moduleCache: false })`, lấy `default` nếu có, rồi kiểm tra bằng Zod.

**Schema Zod** (`domain/codefast-config.schema.ts`, adapter `zod-codefast-config-schema.adapter.ts`) — dạng `CodefastConfig` strict:
...
- Phát hiện workspace **pnpm:** một nguồn truth trong shell — `WorkspacePackageLayoutPort` (hiện thực `NodePnpmWorkspacePackageLayoutAdapter`: đọc `pnpm-workspace.yaml`, glob `package.json`, loại trừ `!…`); miền **mirror** dùng `WorkspacePackageDiscoveryAdapter` ánh xạ sang đường dẫn tương đối cho sync; miền **tag** dùng cùng port cho ứng viên workspace. **Không** nhân đôi logic YAML/glob giữa mirror và tag.
...
**Test tích hợp bắt buộc (DoD `mirror`):** port hoặc viết lại tương đương các kịch bản trong `old/tests/integration/mirror/run-mirror-sync.integration.test.ts` (ghi `exports`, layout `dist/`, lỗi gói, …), cộng `mirror-command.integration.test.ts`; các port `PackageFilterPathResolverPort` và `WorkspacePackageDiscoveryPort` được cover gián tiếp qua `RunMirrorSyncUseCase` — thiếu **nhánh tương đương** các case “hấp dẫn” trong các file đó thì DoD [9.2](#9-2-definition-of-done) `mirror` chưa đạt.
...
Trong `Module.create` của từng miền, dùng `builder.bind(PortToken).to(AdapterClass).singleton()` (hoặc `transient` khi có lý do). **Không** dùng `.scoped()` trên cây CLI (root). **Đăng ký lệnh** (`CliCommandToken` → `*Command`, `whenNamed` + `singleton` — [mục 6.4](#6-4-multibind-clicommand)) gom **một chỗ** trong `bootstrap/cli-application.module.ts` (module ứng dụng gốc), không rải trong từng `domains/<miền>/*.module.ts`. **Cấm** thêm `bind(ArrangeCommand).toSelf()` (hoặc tương đương) chỉ để phục vụ `resolve(ArrangeCommand)` — không có trong thiết kế chuẩn.
...
- `container.load(CliApplicationModule)` — **`CliApplicationModule`** (`bootstrap/cli-application.module.ts`) `import` các module miền (`ArrangeModule`, `MirrorModule`, `TagModule`, …) theo thứ tự phụ thuộc và đăng ký mọi **`CliCommandToken`** tại đây; sau này nếu có `ConfigModule`, module ứng dụng gốc có thể `import(ConfigModule, …)` hoặc glue `load` thêm khi cần.
- Không cặp “presentation + lõi” cho cùng một miền — chỉ một `{miền}.module.ts` / miền.
- Sau `load`, danh sách lệnh lấy bằng `container.resolveAll(CliCommandToken)` ([mục 6.4](#6-4-multibind-clicommand)) — helper `resolveCliCommands` có thể sort theo `name` cho thứ tự ổn định.
...
- **Chỉ** `b.bind(CliCommandToken).to(ArrangeCommand).whenNamed(CLI_COMMAND_SLOT_NAME.arrange).singleton()` trong `bootstrap/cli-application.module.ts` (cùng chỗ cho mọi lệnh). Tên slot dùng chung **`CLI_COMMAND_SLOT_NAME`** (`shell/contracts/cli-command-slots.ts`) với `CliCommand.name` trên lớp lệnh — tránh lệch chuỗi magic. **`toAlias` không dùng** cho đăng ký lệnh; không bind thêm `ArrangeCommand` như token riêng.
- **`bootstrap`:** `load(CliApplicationModule)` rồi `const commands = container.resolveAll(CliCommandToken)` — thêm lệnh mới = bind thêm một dòng trong `cli-application.module.ts` và slot tương ứng trong `CLI_COMMAND_SLOT_NAME`.
...
```typescript
// bootstrap/cli-application.module.ts — sau import các module miền
b.bind(CliCommandToken).to(ArrangeCommand).whenNamed(CLI_COMMAND_SLOT_NAME.arrange).singleton();
```

```typescript
// bootstrap/cli-application.module.ts (tiếp)
b.bind(CliCommandToken).to(MirrorCommand).whenNamed(CLI_COMMAND_SLOT_NAME.mirror).singleton();
```

```typescript
// bootstrap/cli-application.module.ts (tiếp)
b.bind(CliCommandToken).to(TagCommand).whenNamed(CLI_COMMAND_SLOT_NAME.tag).singleton();
```

```typescript
// bootstrap/composition-root.ts — sau container.load(CliApplicationModule)
...
**Đối chiếu snapshot:** trong `old/` có thể thấy `load(ArrangePresentationModule, ArrangeModule)` và glue resolve theo class — **`src/` mới** `load(CliApplicationModule)` (module đó `import` từng `ArrangeModule`, …) và **một** chiến lược token ([6.1](#6-1-modules-per-domain)).
    ...
    config/
      ...
      infrastructure/
        adapters/
          ...
          zod-codefast-config-schema.adapter.ts
    arrange/
      arrange.module.ts
      domain/
        constants.domain.ts
...
- **`shell/infrastructure/`:** nhóm theo **công nghệ / biên hạ tầng** (`node/` cho adapter Node CLI, `workspace/` cho repo + pnpm, `telemetry/` cho `CliPortTelemetryPort`), tệp đơn lẻ khi chỉ một lớp (ví dụ duyệt nguồn `.ts`/`.tsx`). **Không** lồng thư mục kiểu `…/source-code/infrastructure/` (trùng tầng “in-fr” và gợi thêm một slice EA giả trong `shell`).
