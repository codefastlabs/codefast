...
| File                          | Role                                                                                                                                   |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `command.ts`                  | Declares the command's `CommandPipeline` descriptor and wires it with `applyCommandPipeline`; no control flow or serialization inline. |
...
| `output.ts`                   | Human presenters (`present*`) — side effects through `logger`, returning `void`.                                                       |
...
The control flow every command repeats — parse global options, `prepare`, guard, build and validate the request, run,
then report to one of two audiences and record **one** exit code — lives once in
[`core/cli/command-pipeline.ts`](src/core/cli/command-pipeline.ts). A `command.ts` supplies a descriptor of typed slots
(`prepare`, `schema`, `buildRequest`, `run`, `presentHuman` / `formatJson`, `exitCode`, plus optional `guard` and
`createPresenter`); the pipeline computes the exit code once, so `present*` functions return `void`.
...
at the command root; `domain/` is reserved for what is pure. A command whose I/O logic grows past a handful of root
files groups it by subsystem instead of leaving a flat pile — `tag` keeps target discovery under
[`tag/target/`](src/tag/target/) and the `@since` stamping under [`tag/writer/`](src/tag/writer/), with only the shared
vocabulary in `domain/types.ts`.
...
slice — its `run.ts`, `cli-schema.ts`, `output.ts`, `cli-result.ts`, `domain/` — and registers through
`registerPipelineSubcommand`. The parent `command.ts` declares the descriptors and wires them; whatever they share (the
`audit` prelude, `arrange`'s grouping engine under `domain/`) stays at the command root. `arrange group` is the one
command that skips the pipeline: a pure token-string transform with no workspace prelude and a variadic positional, it
stays wired by hand. `mirror`, `pack-slim` and `tag` have no subcommands, so they stay flat.
...
| [`core/`](src/core/)               | Shared primitives: `Result` / `AppError` / `messageFrom`, filesystem (`nodeFilesystem`), workspace discovery, Zod `parseWithSchema`, `logger`, `consumeCliAppError`, path-lesson text edits, TypeScript file walking, the structural view every walker reads `oxc-parser` output through ([`oxc-node.ts`](src/core/oxc-node.ts)) and the line/offset helpers the audits report with ([`source-position.ts`](src/core/source-position.ts)). [`core/cli/`](src/core/cli/) holds the command runtime: the shared [`command-pipeline.ts`](src/core/cli/command-pipeline.ts), global options, positional/result handling, and [`resolve-root.ts`](src/core/cli/resolve-root.ts).                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| [`tag/`](src/tag/)                 | `@since` TSDoc tagging: the pipeline descriptor and `prepare` / `run` / presenters at the root, target discovery and per-target run under [`tag/target/`](src/tag/target/), the `@since` stamping under [`tag/writer/`](src/tag/writer/), shared types and pure filters in [`domain/`](src/tag/domain/).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
