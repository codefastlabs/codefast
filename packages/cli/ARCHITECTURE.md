...
| [`audit/`](src/audit/)             | Read-only audits ([`command.ts`](src/audit/command.ts), [`domain/`](src/audit/domain/)): `audit rtl` scans source for physical-direction Tailwind classes ([`run.ts`](src/audit/run.ts)); `audit links` scans markdown for cross-references that resolve to nothing ([`run-links.ts`](src/audit/run-links.ts)); `audit comments` checks doc-comment conventions ([`run-comments.ts`](src/audit/run-comments.ts)); `audit imports` enforces a rule list ([`import-policy.ts`](src/audit/domain/import-policy.ts) — React by-name, Zod namespace in front-end packages) over each import declaration ([`run-imports.ts`](src/audit/run-imports.ts)), the tier for policies oxlint cannot express. Add a library by adding a rule. |
