...
`@codefast/tracking` requires Node.js 22.12 or later, and TypeScript 7 or later for its types. Its one runtime
dependency is `@standard-schema/spec`. Every peer is optional, needed only by the surface that uses it: `react` and
`react-dom` (19 or later) for the `react/*` subpaths, `@tanstack/react-start` (1.168 or later) for
