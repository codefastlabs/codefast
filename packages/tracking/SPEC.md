...
function createClientTracker<Catalog extends EventCatalog>(catalog: Catalog) {
  ...
  return {
    track<Name extends keyof ClientEvents>(name: Name, properties: z.infer<ClientEvents[Name]["schema"]>): void {
      catalog[name].schema.parse(properties);
      // ...enqueue/send
    },
  };
}
...
│   ├── client/          # createClientTracker, queue, lifecycle, router, gpc, storage, createIsAnalyticsAllowed, createConsentWithdrawalHandler
...
- Supported subpath exports: the intentional barrels (`.`, `./client`, `./server`, `./destinations`, `./react`, `./core`) plus selected deep paths for tree-shake targets (e.g. `./destinations/google-analytics`, `./react/use-consent`), and `./css/*` — see `package.json#exports`. Package-private modules (`client/queue`, `destinations/shared`, `destinations/google-consent`) are reachable only via `#/` imports inside the package.
