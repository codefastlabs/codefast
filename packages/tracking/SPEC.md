...
- **Caller-owned, not package-owned**: `createClientTracker`/`createServerTracker` take `anonymousId` as a required option (a string or, on the client, a `() => string` resolver invoked per event) — the package never mints or persists an ID itself. The app is responsible for generating a stable visitor ID and making it readable on both client and server (e.g. a cookie), so the same ID joins client and server events downstream.
...
- **GA4 name translation**: `TrackedEvent` is a discriminated union on `type: "track" | "page" | "identify" | "group" | "alias"` (not `$`-prefixed magic names), and both GA4 destinations `switch` on `event.type` to translate each kind into GA4's vocabulary — `group` → the recommended `join_group` event (`group_id` param); `identify` → `gtag('set', { user_id })` (client) / dropped on MP (`user_id` already rides in the payload); `alias` → dropped (GA4 merges via `user_id`); `page` → dropped by default on gtag, because `gtag('config')` + Enhanced Measurement (on by default in GA4 admin) already report page views — `trackPageViews: true` opts into a `page_view` translation for apps that disabled both. `track` names that fail GA4's naming rules (must start with a letter, ≤40 chars) are warned about and dropped rather than sent to nowhere.
...
│   ├── destinations/    # http-destination (generic building block), vercel-analytics, google-analytics, ga4-measurement-protocol; posthog.ts not built (Phase 2)
│   ├── react/            # useConsent, ConsentBanner + ConsentToggle (both in consent-banner.tsx) — headless, no @codefast/ui dependency
