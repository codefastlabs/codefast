...
1. **`us` as one bucket is the top risk.** US privacy law is state-level, not federal: 20+ states have in-force comprehensive laws with _divergent_ rules. As of 2026-01-01, roughly **a dozen states legally mandate honoring a universal opt-out signal (GPC)** — CA, CO, CT, DE, MD, MN, MT, NE, NH, NJ, OR, TX — while others (VA, UT) do not (this count is itself a moving target: MD's mandate may be permissive, MT's may have been repealed by 2025 SB 297 — see §3). Sensitive-data handling splits opt-out-to-limit (CA) vs opt-in (VA/CO/CT/TX/OR/MT). A single `us` rule cannot encode this, and missing GPC in a mandate state is an active enforcement target (California AG, Sephora settlement $1.2M, 2022-08-24; CPPA Todd Snyder order $345,178, effective 2025-05-01).
2. **No universal-opt-out-signal handling exists at all.** GPC binds in 12+ US states; California from 2026-01-01 additionally requires a _visible confirmation_ that the signal was processed (11 CCR §7025).
...
| **Montana** MCDPA        | opt-out                                            | **CONTESTED** (see note below)                               | **opt-in**                                                                   | 2025 SB 297 lowered applicability thresholds and may have repealed the UOOM mandate                 |
...
20 states have enacted comprehensive laws; ~12 mandate GPC as of 2026-01-01 (list in §1). GPC's binding force rests on regulator enforcement and §7025's text, not appellate precedent — **flag for counsel on litigation-risk framing.** Nevada (SB220 opt-out of sale + SB370 health, with a 1,750-ft geofence ban that binds even with consent) is a narrower regime — **UNCERTAIN.**

> **Montana UOOM conflict — UNCERTAIN, needs counsel review.** The original 2023 MCDPA required honoring an opt-out preference signal from 2025-01-01, which is why Montana appears in the 12-state list. But **2025 SB 297 (Ch. 567) appears to have repealed and replaced the controller-duties section** — a direct read of the current _Montana Code Annotated 2025_ (§§ 30-14-2808/-2811/-2812) surfaces no universal-opt-out-mechanism mandate, while several practitioner trackers assert it was retained. The statutory text and the trackers conflict. Until resolved against the printed MCA, treat Montana's GPC obligation as **contested**, not settled — this is exactly the kind of newly-amended divergence a per-state verification pass must catch, and a caution that the "12-state" count itself is a moving target.
