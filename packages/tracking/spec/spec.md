...
- The root `.` entry re-exports only the **isomorphic core**; the browser surface (tracker, consent runtime, React bindings, client destinations) and server lanes are imported from their own subpaths, so no lane (or its peer types, e.g. React) couples into a consumer that doesn't use it.
