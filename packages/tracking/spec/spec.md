...
- The server lane is a TanStack Start server function; `./server` and `./tanstack-start` subpaths are denied in client bundles via `SERVER_ONLY_SUBPATHS` (no runtime guard; `codefast mirror` owns the exports map — the group entries are hand-curated under its preserve mode).
