...
export const builtInLoaderConfigs: LoaderConfig[] = [
  ...
  { loader: cloudfrontLoader, matcher: (src) => isDomainMatch(src, 'cloudfront.net') },
  { loader: supabaseLoader, matcher: (src) => isDomainMatch(src, 'supabase.co') },
  { loader: contentfulLoader, matcher: (src) => isDomainMatch(src, 'ctfassets.net') },
  { loader: imagekitLoader, matcher: (src) => isDomainMatch(src, 'imagekit.io') },
  { loader: sanityLoader, matcher: (src) => isDomainMatch(src, 'cdn.sanity.io') },
  { loader: pixelbinLoader, matcher: (src) => isDomainMatch(src, 'pixelbin.io') },
  {
    loader: cloudflareLoader,
    matcher: (src) => isDomainMatch(src, 'cloudflare.com') || isPathMatch(src, '/cdn-cgi/image/'),
  },
  { loader: fastlyLoader, matcher: (src) => isDomainMatch(src, 'fastly.com') || isDomainMatch(src, 'fastlylb.net') },
  { loader: gumletLoader, matcher: (src) => isDomainMatch(src, 'gumlet.io') },
  { loader: imageengineLoader, matcher: (src) => isDomainMatch(src, 'imageengine.io') || isPathMatch(src, 'imgeng') },
  { loader: sirvLoader, matcher: (src) => isDomainMatch(src, 'sirv.com') },
  { loader: thumborLoader, matcher: (src) => isPathMatch(src, 'thumbor') },
];
