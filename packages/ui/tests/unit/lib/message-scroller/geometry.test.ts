...
function stubComputedStyle() {
  ...
  window.getComputedStyle = ((element: Element) => {
    ...
    if (element instanceof HTMLElement && element.dataset.paddingStart) {
      // Only the padding/gap props are read; return a plain stub instead of
      // spreading the live CSSStyleDeclaration (jsdom rejects prop writes on it).
      return {
        gap: "0px",
        rowGap: "0px",
...
function createItems(specs: Array<{ id: string; anchor?: boolean }>): Array<HTMLElement> {
