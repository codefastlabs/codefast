...
- **Node ≥ 22.12** — `engines.node` is `>=22.12.0`, and the package keeps its own `Map` upsert helpers
  ...
  `["ES2024", "ESNext.Decorators"]`, so an ES2025 builtin is a compile error here even on an engine that ships it, where
  runtime alone would never catch the floor violation. Reaching for one means proving the floor's Node ships it and
  widening `lib` deliberately.
