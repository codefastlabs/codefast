...
**The chain is two objects.** `bind()` returns a `BindingEntry` carrying only `to*()`; `to*()` returns a `BindingChain` carrying the refinements and committing to the registry itself. Collapsing them into one instance would put `when*()`/`singleton()` on the object `bind()` hands back, which `tests/unit/container/bind-to-builder-order.test.ts` forbids — it asserts those methods are _absent_, so an out-of-order call is a missing method and not just a type error. Note that SPEC §2.4 only claims the compiler enforces the ordering; the runtime guarantee is the test's own, stricter reading, kept because it is the only thing protecting a JavaScript consumer. Revisit the test, not just the code, if you want one object.

Worth knowing before trying again: the chain's per-bind allocation was measured against a floor that registers the binding with no builder objects at all. The ceiling for removing _every_ builder object is ~19% on `realistic-graph-cold-resolve` at a forced GC every 4096 iterations — the operating point that reproduces the published 0.80×. Removing just the separate committer object (one of four) moved nothing measurable. Measure that floor in-process before believing any object-counting argument here: an in-process A/B against it is the only version of this measurement that came out reproducible — a cross-build A/B of the same change, without an in-run control scenario, reported a clean +4% that was entirely machine drift.
