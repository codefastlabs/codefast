...
const LoggerToken = token<{ line: string }>("decorators-spec-logger");

// Case B — accessor field injection (supported)
@injectable([])
class Dashboard {
  constructor() {}

  // @ts-expect-error — SWC/TS emit `inject(token)` as accessor factory; runtime merges descriptor + callable.
  @inject(LoggerToken) accessor logger!: { line: string };
}
...
describe("TC39 decorators + metadata (SPEC §6)", () => {
  ...
  it("Case B: @inject on accessor writes accessor metadata (TC39 Symbol.metadata)", () => {
    ...
    expect((bucket as { token: unknown }[])[0]?.token).toBe(LoggerToken);
  });
