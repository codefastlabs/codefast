...
// Frames are public objects whose shape the engine's hot loads depend on, so the link back to the
// binding lives beside them, read only when a seeded path is marked.
const frameBindings = new WeakMap<ResolutionFrame, Binding>();

/**
 * Records which binding a resolver-built frame stands for, so a seeded path can be marked from its frames.
 */
export function linkFrameBinding(frame: ResolutionFrame, binding: Binding): void {
  frameBindings.set(frame, binding);
}

/**
 * The bindings behind a stack of resolver-built frames, in stack order.
 *
 * @remarks A frame built outside the resolver was never linked and is left out.
 */
export function bindingsOf(frames: ReadonlyArray<ResolutionFrame>): Array<Binding> {
  const bindings: Array<Binding> = [];
  for (let index = 0; index < frames.length; index += 1) {
    const binding = frameBindings.get(frames[index]!);
