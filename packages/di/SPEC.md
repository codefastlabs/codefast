...
Section này đối chiếu toàn bộ public API của InversifyJS v8.0.0 (phát hành tháng 3/2026) với `@codefast/di`. Mỗi nhóm tính năng được xét theo ba chiều: **học từ v8** (giữ ý tưởng, có thể đổi tên hoặc cải thiện), **cải thiện hơn v8** (v8 có thiếu sót, thư viện này giải quyết), **không học từ v8** (chủ động loại bỏ, có lý do).

---

### 13.1 Bảng so sánh API theo nhóm
...
| Node.js tối thiểu  | Node ≥ 20.19.0 (require(esm)); Node ≥ 18 không có require(esm) | Node ≥ 22.0.0                                     |
| Naming convention  | unqualified = sync, `Async` suffix = async                     | Học từ v8 — giữ nguyên convention                 |

#### Service Identifier

| Khía cạnh                      | InversifyJS v8                                                                  | `@codefast/di`                                                            |
| ------------------------------ | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Kiểu identifier                | `ServiceIdentifier<T>` = `string \| symbol \| AbstractNewable<T> \| Newable<T>` | `Token<Value>` branded type hoặc `Constructor<Value>`                     |
| Type safety                    | Không branded — `container.get<WrongType>('id')` compile được                   | Branded — `resolve(token)` luôn trả đúng type, không thể truyền sai token |
| String identifier              | `container.bind('MyService').to(...)` — `T` infer từ generic                    | Không dùng string — `const My = token<MyService>('MyService')`            |
| Symbol identifier              | `Symbol.for('Ninja')` — type unsafe                                             | Không dùng bare symbol — dùng `token<T>('name')`                          |
| Class identifier               | `container.bind(MyClass).to(...)` — dùng trực tiếp                              | `container.bind(MyClass).to(...)` — giữ nguyên                            |
| Abstract class identifier      | `container.bind(AbstractClass).to(...)` — không enforce                         | TypeScript error — dùng `Token<Value>` thay thế                           |
| Lazy identifier (circular dep) | `LazyServiceIdentifier<T>` — wrapper defer evaluation                           | Không có — dùng `accessor` property injection thay thế                    |

#### Container — tạo và cấu hình

| Khía cạnh         | InversifyJS v8                                   | `@codefast/di`                                                          |
| ----------------- | ------------------------------------------------ | ----------------------------------------------------------------------- |
| Khởi tạo          | `new Container(options?)`                        | `Container.create()` — static factory, không `new`                      |
| Tạo từ modules    | Không có                                         | `Container.fromModules(...modules)` / `Container.fromModulesAsync(...)` |
| Container options | `{ parent?, autobind?, defaultScope? }`          | Không có options — tránh hidden behavior                                |
| Parent container  | `new Container({ parent: parentContainer })`     | `container.createChild()` — explicit hơn                                |
| Auto-bind global  | `new Container({ autobind: true })` hoặc per-get | Không support — "Zero magic" principle                                  |
| Default scope     | `new Container({ defaultScope: 'Singleton' })`   | Không support — tránh hidden global state                               |
| Plugin system     | `container.register(PluginClass)`                | Không có                                                                |
| Snapshot/Restore  | `container.snapshot()` / `container.restore()`   | Không có — dùng module composition + `rebind()` trong test              |

#### Container — binding management

| Method               | InversifyJS v8                                                                     | `@codefast/di`                                                                                 |
| -------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Bind                 | `container.bind<T>(serviceId)` → `BindToFluentSyntax<T>`                           | `container.bind(token)` → `BindToBuilder<Value>` — type inferred từ token                      |
| Unbind theo token    | `container.unbind(serviceId)` — xóa tất cả                                         | `container.unbind(token)` — xóa tất cả slot của token                                          |
| Unbind theo ID       | `container.unbind(bindingId)`                                                      | `container.unbind(bindingId)` — giữ nguyên                                                     |
| Unbind async         | `container.unbindAsync(identifier)`                                                | `container.unbindAsync(identifier)` — giữ nguyên                                               |
| Unbind tất cả        | `container.unbindAll()` / `container.unbindAllAsync()`                             | `container.unbindAll()` / `container.unbindAllAsync()` — giữ nguyên                            |
| Rebind               | `container.rebind<T>(serviceId)` — unbind rồi bind lại, không throw nếu chưa bound | `container.rebind(token)` — throw `RebindUnboundTokenError` nếu token chưa có own binding      |
| Rebind async         | `container.rebindAsync<T>(serviceId)` → `Promise<BindToFluentSyntax<T>>`           | Không có — dùng `unbindAsync()` rồi `bind()` (tách rõ hai bước)                                |
| Load module          | `container.load(...modules: ContainerModule[])`                                    | `container.load(...modules: SyncModule[])`                                                     |
| Load async           | `container.loadAsync(...modules)`                                                  | `container.loadAsync(...modules: Array<SyncModule \| AsyncModule>)`                            |
| Unload module        | `container.unload(...modules)` / `container.unloadAsync(...modules)`               | `container.unload(...modules)` / `container.unloadAsync(...modules)` — thêm reference-counting |
| Load auto-registered | Không có (dùng `autobind`)                                                         | `container.loadAutoRegistered(registry: AutoRegisterRegistry)`                                 |

#### Container — resolution

| Method                    | InversifyJS v8                                                              | `@codefast/di`                                                               |
| ------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Sync resolve              | `container.get<T>(serviceId, options?)`                                     | `container.resolve(token, hint?)`                                            |
| Async resolve             | `container.getAsync<T>(serviceId, options?)`                                | `container.resolveAsync(token, hint?)`                                       |
| Optional sync             | `container.get<T>(serviceId, { optional: true })` → `T \| undefined`        | `container.resolveOptional(token, hint?)` → `T \| undefined`                 |
| Optional async            | `container.getAsync<T>(serviceId, { optional: true })`                      | `container.resolveOptionalAsync(token, hint?)`                               |
| Resolve tất cả            | `container.getAll<T>(serviceId, options?)`                                  | `container.resolveAll(token, hint?)`                                         |
| Resolve tất cả async      | `container.getAllAsync<T>(serviceId, options?)`                             | `container.resolveAllAsync(token, hint?)`                                    |
| Resolve theo name         | `container.get(id, { name: 'file' })`                                       | `container.resolve(token, { name: 'file' })`                                 |
| Resolve theo tag          | `container.get(id, { tag: { key: 'fuel', value: 'petrol' } })` — single tag | `container.resolve(token, { tags: [['fuel', 'petrol']] })` — array of tuples |
| Resolve nhiều tag         | Không có native — phải dùng custom constraint                               | `container.resolve(token, { tags: [['fuel', 'petrol'], ['size', 'v8']] })`   |
| Auto-bind per-resolve     | `container.get(MyClass, { autobind: true })`                                | Không có                                                                     |
| Chained resolution        | `container.getAll(id, { chained: true })` — leo lên parent chain            | Không có — luôn leo lên tự động                                              |
| Async contamination error | Generic error                                                               | `AsyncResolutionError` với `asyncSourceToken` field — chỉ đích danh nguồn    |

#### Container — introspection và lifecycle

| Method                       | InversifyJS v8                                                 | `@codefast/di`                                                       |
| ---------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------- |
| Check binding (hierarchy)    | `container.isBound(serviceId, options?)`                       | `container.has(token, hint?)`                                        |
| Check binding (own only)     | `container.isCurrentBound(serviceId, options?)`                | `container.hasOwn(token, hint?)`                                     |
| Lookup bindings              | Không có                                                       | `container.lookupBindings(token)` → `readonly Binding<Value>[]`      |
| Snapshot                     | `container.inspect()` → typed snapshot với `ContainerSnapshot` | `container.inspect()` → `ContainerSnapshot` (tương tự)               |
| Dependency graph             | Không có                                                       | `container.generateDependencyGraph(options?)` → `ContainerGraphJson` |
| Container-level activation   | `container.onActivation(serviceId, handler)`                   | `container.onActivation(token, handler)` — giữ nguyên                |
| Container-level deactivation | `container.onDeactivation(serviceId, handler)`                 | `container.onDeactivation(token, handler)` — giữ nguyên              |
| Validate dependency graph    | Không có                                                       | `container.validate()` — detect captive dependency tại static-time   |
| Warm up singletons           | Không có                                                       | `container.initializeAsync()` — idempotent, fail-fast khi startup    |

#### Container — disposal

| Method                  | InversifyJS v8 | `@codefast/di`                                                                    |
| ----------------------- | -------------- | --------------------------------------------------------------------------------- |
| Dispose                 | Không spec     | `container.dispose()` → `Promise<void>`                                           |
| `Symbol.asyncDispose`   | Không spec     | `container[Symbol.asyncDispose]()` — `await using` support                        |
| `Symbol.dispose`        | Không spec     | `container[Symbol.dispose](): never` — luôn throw `SyncDisposalNotSupportedError` |
| Child container dispose | Không spec     | `await child.dispose()` — chỉ deactivate binding của child                        |

#### Binding strategies

| Strategy                  | InversifyJS v8                                                                       | `@codefast/di`                                                                   |
| ------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| Class binding             | `.to(Class)` → `BindInWhenOnFluentSyntax<T>`                                         | `.to(Class)` → `BindingBuilder<Value>` — giữ nguyên                              |
| Self binding              | `.toSelf()` → `BindInWhenOnFluentSyntax<T>`                                          | `.toSelf()` → `BindingBuilder<Value>` — giữ nguyên                               |
| Constant value            | `.toConstantValue(value)` → `BindWhenOnFluentSyntax<T>`                              | `.toConstantValue(value)` → `ConstantBindingBuilder<Value>` — giữ nguyên         |
| Dynamic factory (unified) | `.toDynamicValue(ctx => T \| Promise<T>)` — một method cho cả sync lẫn async         | Không có — tách thành hai method riêng biệt                                      |
| Dynamic factory (sync)    | Không có API riêng — dùng `toDynamicValue`                                           | `.toDynamic(ctx => T)` → `BindingBuilder<Value>`                                 |
| Dynamic factory (async)   | Không có API riêng — dùng `toDynamicValue`                                           | `.toDynamicAsync(ctx => Promise<T>)` → `BindingBuilder<Value>`                   |
| Resolved value            | `.toResolvedValue(factory, injectOptions?)` — inject options hỗ trợ name/tag per dep | `.toResolved(factory, deps)` — plain token array, không hỗ trợ name/tag per dep  |
| Factory (curried)         | `.toFactory(ctx => curriedFn)` → `BindWhenOnFluentSyntax<T>`                         | Không có — dùng `toConstantValue(fn)` hoặc `toDynamic`                           |
| Alias / service           | `.toService(otherId)` → `void`                                                       | `.toAlias(otherToken)` → `AliasBindingBuilder<Value>` — có constraint và `.id()` |

> **Tại sao tách `toDynamic` / `toDynamicAsync`?** `toDynamicValue` trong v8 nhận cả sync lẫn async factory nhưng compiler không enforce việc dùng `getAsync()`. Khi factory return `Promise`, gọi `container.get()` (sync) không phát hiện lỗi cho đến runtime. `toDynamicAsync` buộc dùng `resolveAsync()` — compiler detect tại compile time thông qua async contamination.

> **Tại sao không có `toFactory`?** `toFactory` tạo ra một layer indirection: inject factory function, rồi gọi factory để tạo instance thật. Pattern này phức tạp hơn `toConstantValue(fn)` (inject fn trực tiếp) hay `toDynamic(ctx => ...)` (factory inline). Tránh indirection là nguyên tắc của thư viện này.

#### Binding scope

| Scope                               | InversifyJS v8                                                                                | `@codefast/di`                                                                                         |
| ----------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Singleton                           | `.inSingletonScope()` → `BindWhenOnFluentSyntax<T>`                                           | `.singleton()` → `SingletonBindingBuilder<Value>` — có `onDeactivation`                                |
| Transient                           | `.inTransientScope()` → `BindWhenOnFluentSyntax<T>`                                           | `.transient()` → `TransientBindingBuilder<Value>` — không có `onDeactivation`                          |
| Request / Scoped                    | `.inRequestScope()` → `BindWhenOnFluentSyntax<T>` — per-resolve-tree (trong một lệnh `get()`) | `.scoped()` → `ScopedBindingBuilder<Value>` — per-child-container                                      |
| Scope semantics khác biệt           | `inRequestScope`: singleton trong một resolve tree; mỗi `container.get()` là một tree mới     | `scoped`: singleton trong phạm vi một child container; child container là lifecycle boundary           |
| `onDeactivation` trên non-singleton | Runtime error nếu thêm `onDeactivation` vào transient/request binding                         | TypeScript error — `onDeactivation` không expose trên `TransientBindingBuilder`/`ScopedBindingBuilder` |

> **`inRequestScope` vs `scoped` — khác biệt căn bản:** `inRequestScope` trong InversifyJS cô lập instance trong phạm vi một lệnh `container.get()`. Tạo hai lệnh `get()` độc lập = hai instance khác nhau. `scoped` trong thư viện này cô lập instance trong phạm vi một child container — child container là ranh giới, không phải lệnh resolve. Đây là lý do thư viện dùng tên khác để tránh nhầm lẫn về semantics.

#### Binding constraints (`when*`)

| Constraint              | InversifyJS v8                                                                                                         | `@codefast/di`                                                                                                                                  |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Thứ tự trong chain      | Flexible — `when*` khả dụng cả trước và sau scope (vì type là union)                                                   | Strict — `when*` chỉ sau `to*()`, trước scope; scope builders không expose `when*`                                                              |
| Custom predicate        | `.when(constraint: BindingConstraints => boolean)`                                                                     | `.when(predicate: ConstraintContext => boolean)`                                                                                                |
| Predicate context       | `BindingConstraints` — `name`, `tag`, ancestry metadata đơn giản                                                       | `ConstraintContext` — `resolutionPath`, `materializationStack`, `parent`, `ancestors`, `currentResolveHint`                                     |
| Named constraint        | `.whenNamed(name: MetadataName)`                                                                                       | `.whenNamed(name: string)` — giữ nguyên                                                                                                         |
| Tagged constraint       | `.whenTagged(tag: MetadataTag, tagValue: unknown)` — một tag                                                           | `.whenTagged(tag: string, value: unknown)` — một tag; nhiều tag qua chain `.whenTagged(...).whenTagged(...)`                                    |
| Default constraint      | `.whenDefault()`                                                                                                       | `.whenDefault()` — giữ nguyên                                                                                                                   |
| Parent constraint       | `.whenParent(constraint)`, `.whenParentIs(serviceId)`, `.whenParentNamed()`, `.whenParentTagged()`                     | `whenParentIs(token)`, `whenParentNamed()`, `whenParentTagged()` — export từ `@codefast/di/constraints`                                         |
| Ancestor constraint     | `.whenAnyAncestor(constraint)`, `.whenAnyAncestorIs(serviceId)`, `.whenAnyAncestorNamed()`, `.whenAnyAncestorTagged()` | `whenAnyAncestorIs(token)`, v.v. — từ `@codefast/di/constraints`                                                                                |
| No-parent constraint    | `.whenNoParent(constraint)`, `.whenNoParentIs(serviceId)`, `.whenNoParentNamed()`, `.whenNoParentTagged()`             | Từ `@codefast/di/constraints`                                                                                                                   |
| No-ancestor constraint  | `.whenNoAncestor(constraint)`, `.whenNoAncestorIs(serviceId)`, v.v.                                                    | Từ `@codefast/di/constraints`                                                                                                                   |
| Vị trí trong public API | Tất cả trên `BindWhenFluentSyntax` — là phần của main API surface                                                      | `whenParent*`/`whenAnyAncestor*`/`whenNoParent*`/`whenNoAncestor*` được export từ subpath `@codefast/di/constraints` — giữ main API surface nhỏ |

> **Tại sao `when*` không khả dụng sau scope trong thư viện này?** Trong InversifyJS v8, `BindWhenOnFluentSyntax` (trả về từ `inSingletonScope()`) vẫn expose `when*`. Điều này cho phép `.to(X).inSingletonScope().whenNamed('a')` — đọc từ trái sang phải không tự nhiên. Thư viện này enforce thứ tự `to* → when* → scope → on*` qua type system: một khi `scope()` được gọi, builder trả về không còn expose `when*`.

#### Binding identifier

| Khía cạnh        | InversifyJS v8                                      | `@codefast/di`                                           |
| ---------------- | --------------------------------------------------- | -------------------------------------------------------- |
| Lấy binding ID   | `.getIdentifier()` trên bất kỳ `BoundServiceSyntax` | `.id()` — tên ngắn hơn, giữ concept                      |
| Dùng để unbind   | `container.unbind(bindingIdentifier)`               | `container.unbind(bindingId)` — giữ nguyên               |
| Branded type     | `BindingIdentifier` — branded                       | `BindingIdentifier` — branded, không thể tạo thủ công    |
| Trên `toService` | Không có — `toService()` trả về `void`              | Có — `toAlias()` trả về `AliasBindingBuilder` có `.id()` |

#### Lifecycle hooks

| Hook                         | InversifyJS v8                                                                                           | `@codefast/di`                                                                            |
| ---------------------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Per-binding activation       | `.onActivation(handler: BindingActivation<T>)` → `BindWhenFluentSyntax<T>` — phải annotate type thủ công | `.onActivation(fn)` — type inferred từ binding, không cần annotate                        |
| Per-binding deactivation     | `.onDeactivation(handler)` — runtime error nếu không phải singleton                                      | `.onDeactivation(fn)` — chỉ trên `SingletonBindingBuilder`, TypeScript error nếu dùng sai |
| Container-level activation   | `container.onActivation(serviceId, handler)`                                                             | `container.onActivation(token, handler)` — giữ nguyên                                     |
| Container-level deactivation | `container.onDeactivation(serviceId, handler)`                                                           | `container.onDeactivation(token, handler)` — giữ nguyên                                   |
| Activation handler type      | `BindingActivation<T>` = `(ctx: ResolutionContext, instance: T) => T \| Promise<T>`                      | `ActivationHandler<Value>` — tương tự, nhưng `ctx` là `ResolutionContext` của thư viện    |
| Deactivation handler type    | `BindingDeactivation<T>` = `(instance: T) => void \| Promise<void>`                                      | `DeactivationHandler<Value>` — tương tự                                                   |
| Thứ tự lifecycle đầy đủ      | `@postConstruct()` → per-binding `onActivation()` → container-level `onActivation()`                     | Giữ nguyên thứ tự này                                                                     |
| Deactivation order           | Ngược activation                                                                                         | Container-level `onDeactivation()` → per-binding `onDeactivation()` → `@preDestroy()`     |

#### Decorator layer

| Decorator / API               | InversifyJS v8                                                                   | `@codefast/di`                                                                      |
| ----------------------------- | -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Metadata mechanism            | `emitDecoratorMetadata` — TypeScript emit constructor param types tự động        | `Symbol.metadata` (TC39 Stage 3) + deps array khai báo tường minh                   |
| `@injectable()`               | `@injectable()` — không cần deps; TypeScript tự emit qua `emitDecoratorMetadata` | `@injectable()` — zero-dep class; `@injectable([Logger, Config])` — khai báo deps   |
| `@inject` trên parameter      | `@inject(serviceId)` trên constructor param — `experimentalDecorators`           | Không có — TC39 Stage 3 không hỗ trợ parameter decorator (TS1206)                   |
| `@inject` trên property       | `@inject(serviceId)` trên plain property                                         | `@inject(token) accessor field` — dùng TC39 `accessor` keyword                      |
| Named injection               | `@named(name)` trên parameter                                                    | `inject(Logger, { name: 'file' })` trong deps array                                 |
| Tagged injection              | `@tagged(tag, value)` trên parameter                                             | `inject(Engine, { tags: [['fuel', 'electric']] })` trong deps array                 |
| Optional injection            | `@optional()` trên parameter                                                     | `optional(Logger)` trong deps array — trả `undefined` nếu không bound               |
| Multi injection               | `@multiInject(serviceId)` trên parameter hoặc property                           | `injectAll(Plugin)` trong deps array — resolve tất cả binding match thành mảng      |
| Inheritance injection         | `@injectFromBase()` — child kế thừa deps của parent                              | Không có — mọi dep phải khai báo tường minh trong child                             |
| Hierarchy injection           | `@injectFromHierarchy()` — leo lên prototype chain                               | Không có — explicit deps thay thế                                                   |
| Unmanaged param               | `@unmanaged()` trên parameter — bỏ qua injection cho arg đó                      | Không có — trong deps array, đơn giản không khai báo arg không cần inject           |
| Decorate third-party          | `decorate(decorator, target, idx?)` — programmatic decoration                    | Không có — không target third-party class integration                               |
| `@postConstruct()`            | Method decorator — chỉ **một** method per class                                  | Method decorator — **nhiều** method per class, gọi theo thứ tự top-down             |
| `@preDestroy()`               | Method decorator — chỉ một method per class                                      | Method decorator — nhiều method per class, gọi theo thứ tự top-down                 |
| `@postConstruct` async        | Hỗ trợ async                                                                     | Hỗ trợ async — async contamination lan truyền dependency path                       |
| Scope hint trên `@injectable` | Không có trong v8                                                                | Không có — scope là binding-time concern, khai báo tại chain                        |
| Auto-register                 | `new Container({ autobind: true })` — global, dựa trên constructor type          | `@injectable([deps], { autoRegister: registry })` — explicit registry, không global |

#### Module system

| Khía cạnh                     | InversifyJS v8                                                                               | `@codefast/di`                                                                  |
| ----------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Module class                  | `new ContainerModule(callback: ContainerModuleLoadOptions => void)`                          | `SyncModule.create(name, builder => void)` — static factory                     |
| Async module                  | `new AsyncContainerModule(callback)` — không phân biệt type level                            | `AsyncModule.create(name, async builder => void)` — branded type                |
| Module phân biệt ở type level | `ContainerModule` và `AsyncContainerModule` — structural typing, không branded               | `SyncModule` / `AsyncModule` — branded, `load(asyncModule)` là TypeScript error |
| Operations trong module       | `bind`, `unbind`, `rebind`, `isBound`, `onActivation`, `onDeactivation` — full container ops | `bind`, `import` — additive-only, không unbind hay rebind                       |
| Module additive-only          | Không — module có thể unbind và rebind binding của module khác                               | Có — `ModuleBuilder` chỉ additive, tránh hidden coupling                        |
| Module deduplication          | Không có native dedup — load nhiều lần = bind nhiều lần                                      | Object identity deduplication — import cùng module nhiều lần là no-op           |
| Reference counting            | Không có                                                                                     | Có — `ModuleB` chỉ bị unbind khi tất cả modules import nó đều bị unload         |
| Module name                   | Không bắt buộc                                                                               | Bắt buộc — dùng trong error message và diagnostics                              |

#### Error handling

| Khía cạnh                            | InversifyJS v8                                                                   | `@codefast/di`                                                                                   |
| ------------------------------------ | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| Typed error hierarchy                | Không có — error chỉ là `Error` với message string                               | `DiError` abstract base + 10 concrete subclasses, mỗi class có `readonly code` và context fields |
| Token không bound                    | Runtime error với message string                                                 | `TokenNotBoundError` — `tokenName` field                                                         |
| Không match constraint/hint          | Runtime error với message string                                                 | `NoMatchingBindingError` — `tokenName`, `hint`, `availableSlots` fields                          |
| Circular dependency                  | Runtime error                                                                    | `CircularDependencyError` — `cycle: string[]` field với đầy đủ vòng lặp                          |
| Async resolution sync                | Runtime error                                                                    | `AsyncResolutionError` — `tokenName`, `asyncSourceToken` — chỉ đích danh nguồn async trong chain |
| Captive dependency (scope violation) | Không detect tĩnh — chỉ phát hiện nếu dùng `validate` plugin (không có built-in) | `ScopeViolationError` — `ScopeViolationDetails` với consumer, dependency, path                   |
| Missing decorator                    | Runtime error                                                                    | `MissingMetadataError` — `targetName` field                                                      |
| Async module trong sync load         | Runtime error                                                                    | `AsyncModuleLoadError` — `moduleName` field                                                      |
| Sync disposal                        | Không spec                                                                       | `SyncDisposalNotSupportedError` — rõ ràng yêu cầu `await using`                                  |
| Missing scope context                | Runtime error chung                                                              | `MissingScopeContextError` — `tokenName` field                                                   |
| Rebind unbound token                 | Không throw — hoạt động như `bind()` mới                                         | `RebindUnboundTokenError` — contract tường minh: `rebind` chỉ thay thế, không tạo mới            |

#### Testing và development workflow

| Khía cạnh                   | InversifyJS v8                                                                  | `@codefast/di`                                                                  |
| --------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Save/restore state          | `container.snapshot()` / `container.restore()` — stack-based                    | Không có — dùng module composition: tạo test module override binding            |
| Override binding trong test | `container.snapshot(); container.rebind(x).toConstantValue(mock); ...restore()` | `const c = Container.fromModules(AppModule); c.rebind(x).toConstantValue(mock)` |
| Validate dependency graph   | Không có built-in                                                               | `container.validate()` — chạy sau load trước khi serve                          |
| Warm up singletons          | Không có                                                                        | `await container.initializeAsync()` — fail-fast khi startup                     |
| Dependency graph export     | Không có                                                                        | `container.generateDependencyGraph()` → JSON + DOT format                       |

---

### 13.2 Tổng hợp: học từ v8
...
| `toService()` alias concept                                    | `toAlias()` — đổi tên rõ hơn, trả về `AliasBindingBuilder` thay vì `void`           |
| `BindingIdentifier` / `.getIdentifier()`                       | Giữ concept, đổi thành `.id()` — tên ngắn hơn                                       |
| `whenNamed` / `whenTagged` / `whenDefault` / `when(predicate)` | Giữ nguyên, tag key là `string` only (không dùng `MetadataTag`)                     |
| `isBound()` check hierarchy                                    | `has()` — giữ semantics                                                             |
| `isCurrentBound()` check current only                          | `hasOwn()` — tên rõ hơn (`isCurrentBound` dễ nhầm với `isBound` current instance)   |
...
| `getAll` filter semantics (v8 thay đổi từ v6)                  | `resolveAll` — filter semantics, trả `[]` khi không match (không throw)             |
| `bind(id).unbind(bindingId)` — unbind một binding cụ thể       | Giữ nguyên qua `container.unbind(bindingId)`                                        |

---

### 13.3 Tổng hợp: cải thiện hơn v8
...
| `when*` khả dụng sau scope (trên `BindWhenOnFluentSyntax`)                      | `when*` không khả dụng sau scope — compiler enforce thứ tự chuẩn               |
| `onDeactivation` không có compile-time guard — runtime error trên non-singleton | Builder type narrowing — `onDeactivation` chỉ trên `SingletonBindingBuilder`   |
| `toService()` trả về `void` — không hỗ trợ constraint hay `.id()`               | `toAlias()` trả về `AliasBindingBuilder` — có `when*` và `.id()`               |
| `@inject` trên parameter cần `experimentalDecorators`                           | `@injectable([deps])` + `inject()` — TC39 Stage 3 thuần                        |
| `@inject` trên plain property                                                   | `@inject accessor field` — dùng TC39 `accessor` keyword (proper getter/setter) |
| `getAll()` chỉ có sync                                                          | `resolveAll()` + `resolveAllAsync()`                                           |
| `container.get()` + `{ optional: true }` — optional ẩn trong options            | `resolveOptional()` + `resolveOptionalAsync()` — tên method tường minh         |
| `tag` trong `GetOptions` là single tag object — không hỗ trợ multi-tag          | `tags` là `ReadonlyArray<readonly [string, unknown]>` — multi-tag lookup       |
...
| Không có `validate()`                                                           | `container.validate()` — detect captive dependency tĩnh                        |
...
| `resolveAll` / `getAll` no-match behavior không rõ                              | Luôn trả `[]` — không throw, filter semantics                                  |
| `autoRegister` qua `autobind` global option hoặc per-get                        | `createAutoRegisterRegistry()` — explicit registry, không global state         |
...
| Kiểu nền tảng thiếu hoặc phân tán                                               | `types.ts` tập trung, export đầy đủ từ `@codefast/di`                          |

---

### 13.4 Tổng hợp: không học từ v8
...
| `snapshot()` / `restore()`                                          | Module composition + `rebind()` thay thế trong test workflow               |
...
_Phiên bản tài liệu: 7.0 — April 2026_
