...
import { twMergeConfig } from "#/tests/unit/fixtures/tailwind-merge-config";
...
describe("Tailwind Variants (TV) - Slots", () => {
  ...
  test("should work with slots -- default variants -- custom class & className", () => {
    ...
    expect(base({ className: "text-lg" })).toHaveClassName(["font-bold", "underline", "bg-blue-500", "text-lg"]);
    expect(base({ className: "text-lg" })).toHaveClassName(["font-bold", "underline", "bg-blue-500", "text-lg"]);
