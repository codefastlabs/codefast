...
const defineButton = (): string =>
  (codefastTvFn(buttonVariants, TV_MERGE_ENABLED) as (props: unknown) => string)({
    size: "sm",
    variant: "outline",
  });

const defineCard = (): string =>
  (codefastTvFn(slotsVariants, TV_MERGE_ENABLED) as (props: unknown) => ServicePreviewSlots)({
    size: "md",
    variant: "info",
  }).base();
 ...
 * @since 0.6.0
 */
export function buildCodefastConstructScenarios(): ReadonlyArray<BenchScenario> {
  return [
    {
      ...CONSTRUCT_SIMPLE,
