...
5. The parent **aggregates** trials into medians and IQR (`@codefast/benchmark-harness`, `report/aggregate.ts`), writes `bench-results/latest.md` and `latest.jsonl` with the di-vs-inversify head-to-head summary on top, and appends the **N-way core-subset** table (di vs inversify vs awilix vs tsyringe).
...
| **codefast / <competitor>**   | Ratio of median **hz/op** (di over that competitor — `codefast / inversify` in the two-way table; `cf/awi`, `cf/tsy` also appear in the N-way table). **> 1×** ⇒ codefast higher throughput on that row; **< 1×** ⇒ the competitor higher. `—` if undefined (e.g. missing data). |
...
- The report opens with a **Head-to-head summary**: win/parity/loss counts over comparable rows (parity band ±3%), the **median** and **geomean** ratios, lists of losses/parity rows, and a **geomean-by-group** line. The per-group breakdown keeps error-path groups (`failure` — fail-fast, cycle detection) visibly separate from throughput, so a single 100×+ row can't read as a typical speedup.
...
After the full di-vs-inversify table, the report appends an **N-way core-subset** table comparing `@codefast/di` (pivot) against **inversify**, **awilix**, and **tsyringe** on the scenarios all four libraries support (factory/class bindings: constant/singleton/transient resolve, the realistic graph, transient chains, fan-out tree). awilix and tsyringe implement only this subset, so they never appear in the full two-way table. Each competitor gets its own throughput column plus a `codefast / competitor` ratio column, and a per-competitor win/parity/loss + median/geomean line. The rendering lives in `@codefast/benchmark-harness` — `src/report/n-way.ts`.
...
| Medians, IQR, head-to-head summary, geomean  | `@codefast/benchmark-harness` — `src/report/aggregate.ts`, `src/report/two-way.ts` |
| N-way core-subset table (di + 3 competitors) | `@codefast/benchmark-harness` — `src/report/n-way.ts`                              |
