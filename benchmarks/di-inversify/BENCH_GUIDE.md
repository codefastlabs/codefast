...
| **codefast / <competitor>**   | Ratio of median **hz/op** (di over that competitor — `@codefast/di / inversify` in the head-to-head table; one ratio column per competitor in the core-subset table). **> 1×** ⇒ codefast higher throughput on that row; **< 1×** ⇒ the competitor higher. `—` if undefined (e.g. missing data). |
...
After the full di-vs-inversify table, the report appends an **N-way core-subset** table comparing `@codefast/di` (pivot) against **inversify**, **awilix**, and **tsyringe** on the scenarios all four libraries support (factory/class bindings: constant/singleton/transient resolve, the realistic graph, transient chains, fan-out tree). awilix and tsyringe implement only this subset, so they never appear in the full head-to-head table. Each competitor gets its own throughput column plus a `codefast / competitor` ratio column, and a per-competitor win/parity/loss + median/geomean line. Both tables come from the same renderer — `@codefast/benchmark-harness`, `src/report/comparison.ts` — the head-to-head being its two-library case.
...
| Core-subset table (di + 3 competitors)      | `@codefast/benchmark-harness` — `src/report/comparison.ts` (`compact` column profile) |
