...
first, on Node 26.1.0 / V8 14.6, Apple M3 Max × 14, darwin/arm64. A narrowed run writes its own timestamped directory
and leaves `latest.*` alone, so the published full-suite state below is untouched; the existing codefast/inversify rows
are unchanged because adding a library changes none of codefast's own measurements.

Per-competitor over the rows each one implements — median of the three passes' medians:

| Competitor   | Comparable | Win / parity / loss | Median | Where `@codefast/di` loses                                                       |
| ------------ | ---------: | ------------------- | -----: | -------------------------------------------------------------------------------- |
| inversify    |     8 of 8 | 8 / 0 / 0           |  2.56× | —                                                                                |
| Awilix 13    |     8 of 8 | 8 / 0 / 0           |  3.51× | —                                                                                |
