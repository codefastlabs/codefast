...
| `RESOLUTION_SET_THRESHOLD = 32` | a depth past which the synchronous cycle check kept a set beside the stack                      | gone: every synchronous lane checks a cycle by the binding's in-flight flag, and a seeded path (a plan's escape, an async level's synchronous call) is marked around the call, idempotently (ported from the research branch; `interpreted-class-chain-40` 5 129 against 6 681 ns, `-24` 2 960 against 3 145, `scale-deep-transient-chain-512` 17.9 against 19.3 µs; `plan-escape-factory-dep` 108 against 99 ns and `scale-mid-transient-chain-32` 669 against 636 pay the marking; `nested-context-resolve-in-factory`, `plan-deps-inlined`, `transient-class-1-dep` flat) | —                                 |
