...
- **Type checking _and_ package builds use the native TypeScript 7 compiler** (`tsc`, the Go port) under the `@typescript/native` catalog alias (`npm:typescript@7`). TS 7 emits both `.js` and `.d.ts`, so every `packages/*` builds with plain `tsc` (no bundler — see below); type-checking is `tsc --noEmit`. The plain `typescript` dependency stays aliased to `@typescript/typescript6` (the classic JS compiler, bin `tsc6`) — still required because **`@codefast/cli` consumes the classic TS compiler API at runtime** (it ships `typescript` as a `dependencies`; its `arrange`/`audit`/`tag` commands use `ts.createSourceFile`, `ts.SyntaxKind`, `ts.factory`, and AST node types). TS 7's public `.` export is only `{ version }` (the compiler API moved to `./unstable/*`), so pointing `typescript` at 7.x makes `ts.isJSDoc`/`ts.TypeAliasDeclaration`/etc. vanish — verified: it breaks `@codefast/cli`'s build (TS2339) and type-aware lint. This is the concrete blocker (the TanStack Start Vite plugins may also rely on it, but the CLI alone is decisive; in the whole repo, only the CLI imports classic `typescript`). **The trigger to drop `@typescript/typescript6` is rewriting the CLI's AST tooling against `typescript/unstable`** (TS 7's compiler surface), not a TS release number. When that lands (and TanStack supports TS 7), point the `typescript` alias at `npm:typescript@7` and collapse the two catalog entries into one.
