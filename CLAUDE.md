...
- **Type checking _and_ package builds use the native TypeScript 7 compiler** (`tsc`, the Go port) under the `@typescript/native` catalog alias (`npm:typescript@7`). TS 7 emits both `.js` and `.d.ts`, so every `packages/*` builds with plain `tsc` (no bundler — see below); type-checking is `tsc --noEmit`. The plain `typescript` dependency stays aliased to `@typescript/typescript6` (the classic JS compiler, bin `tsc6`) — still required because the **TanStack Start Vite plugins** consume the classic TS compiler API (TS 7's public `.` export is only `{ version }`, with the compiler API under `./unstable/*`). **The trigger to drop `@typescript/typescript6` is the TanStack plugins migrating to the `typescript/unstable` API**, not a TS release number. When that lands, point the `typescript` alias at `npm:typescript@7` and collapse the two catalog entries into one.
