          ...
          isCollapsed ? (
            <Tooltip key={link.title} delayDuration={0}>
              <TooltipTrigger asChild>
                <Link className={cn(buttonVariants({ icon: true, variant: link.variant }), 'size-9')} href="#">
            ...
            <Link
              key={link.title}
              className={cn(buttonVariants({ size: 'sm', variant: link.variant }), 'justify-start')}
              href="#"
            >
              <link.icon className="mr-2 size-4" />
              {link.title}
              {link.label ? (
                <span className={cn('ml-auto', link.variant === 'default' && 'text-background')}>{link.label}</span>
              ) : null}
            </Link>
          ),
        )}
