...
const WRONG = `<pre class="shiki"><code>@injectable([ClockToken])</code></pre>`;
...
describe("TypedTokensCard", () => {
  it("opens on the well-declared class and a passing compile", () => {
    ...
    expect(screen.queryByText("@injectable([ClockToken])")).not.toBeInTheDocument();
  ...
  it("swaps in the wrong token and the compiler's cause on demand, and back", async () => {
    ...
    expect(screen.getByText("@injectable([ClockToken])")).toBeInTheDocument();
    expect(screen.getByRole("status")).toHaveTextContent("Property 'info' is missing in type 'Clock'");
