...
export default defineConfig(({ command }) => {
  ...
  return {
    resolve: {
      // `source` (dev only) resolves @codefast/* packages to their `src`
      // entry points for HMR; `module` keeps dual CJS/ESM third-party deps on
      // their ESM build in both dev and prod (avoids the tslib `__extends`
      // SSR interop error). The only dev↔prod difference is the `source`
      // opt-in. @codefast/* packages have no `module` key, so they fall back
      // to the always-on `import` condition (→ dist/*.mjs) in prod.
