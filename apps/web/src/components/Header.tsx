...
export default function Header() {
  return (
    <header className="site-header-nav sticky top-0 z-50 border-b border-[var(--line)]">
      <div className="page-wrap flex h-12 items-center gap-8 px-4">
        ...
        >
          ...
          <span className="text-sm font-semibold tracking-tight text-[var(--sea-ink)]">
            codefast<span className="text-[var(--lagoon)]">/ui</span>
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-0.5 sm:flex" aria-label="Main navigation">
          {NAV_LINKS.map(({ to, label }) => (
            <Link
              key={to}
              to={to}
              className="rounded-lg px-3 py-1.5 text-[13px] text-[var(--sea-ink-soft)] no-underline transition-colors hover:bg-[var(--link-bg-hover)] hover:text-[var(--sea-ink)]"
              activeProps={{
                className:
        ...
        <div className="ml-auto flex items-center gap-1">
          <a
            ...
            className="rounded-lg p-2 text-[var(--sea-ink-soft)] transition-colors hover:bg-[var(--link-bg-hover)] hover:text-[var(--sea-ink)]"
          >
