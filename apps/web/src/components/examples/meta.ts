 ...
 *
 * AUTO-DISCOVERED — every `examples/<slug>.meta.ts` contributes one component;
 * there is no hand-maintained list. The slug comes from the filename and
 * `hasDemo` from whether `<slug>.demo.tsx` exists (the `demos.tsx` glob), so
 ...
 *
 * TO ADD A COMPONENT: create `examples/<slug>.meta.ts` exporting a single
 ...
 * detail page (see `demos.tsx` and `docs.ts`).
 */
...
/** `./button.meta.ts` → `button`. */
function slugFromMetaPath(path: string): string {
  return path.slice(path.lastIndexOf("/") + 1).replace(/\.meta\.ts$/, "");
}
