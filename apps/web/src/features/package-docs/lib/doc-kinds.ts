...
export function docPath(pkg: string, kind: DocKindSlug, page?: string): string {
  if (page) {
    return `/docs/${pkg}/${kind}/${page}`;
  }

  return kind === "readme" ? `/docs/${pkg}` : `/docs/${pkg}/${kind}`;
}

/** The document's name in analytics events, e.g. `tracking/spec/spec-consent` — the site path without `/docs/`. */
export function docAnalyticsName(pkg: string, kind: DocKindSlug, page?: string): string {
  return docPath(pkg, kind, page).slice("/docs/".length);
}
...
export function docRefFor(relativePath: string): DocRef | null {
  ...
  if (slash === -1) {
    ...
    return kind ? { kind: kind.slug } : null;
  }
