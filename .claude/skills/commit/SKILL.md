...
Goal: do NOT commit every file in a single commit. Instead, group changes by logical relatedness and create one commit
per group.
...
2. **Group by relatedness** Cluster files into independent groups by the _meaning_ of the change, not by directory.
   Suggested criteria:
   ...
   - If a single file mixes unrelated changes, split it by hunk **without** interactive git: send `git diff -- <file>`
     to a patch file, delete the hunks belonging to other groups, then `git apply --cached <patch>` to stage only what
     remains. The deleted hunks stay unstaged and are picked up by a later group.
...
4. **Commit each group — automatically, no confirmation** For each group, in a sensible order (e.g. deps/config first,
   then features, then tests, then docs):
...
- Interactive git is unavailable here — no `git add -p`, `git add -i`, or `git rebase -i`. Hunk-level staging goes
  through the `git apply --cached` patch route.
- Do NOT edit code just to "make grouping look nicer"; only edit when check-types reports an error.
- Each commit should compile/type-check independently where possible (don't split commits in a way that breaks
  type-check midway when the changes depend on each other — in that case, put them in the same group).
