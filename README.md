...
- `@codefast/ui`: a stub React component library shared by both `web` and `docs` applications
- `@codefast/eslint-config`: `eslint` configurations (includes `eslint-config-next` and `eslint-config-prettier`)
- `@codefast/typescript-config`: `tsconfig.json`s used throughout the monorepo
