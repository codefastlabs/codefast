...
This script automatically converts Tailwind v3 syntax to v4 equivalents.
...
SIZE_UTILITIES = frozenset({
    'size-', 'w-', 'h-', 'max-w-', 'max-h-', 'min-w-', 'min-h-',
    'max-size-', 'min-size-',
})

SPACING_UTILITIES = frozenset({
    # Padding
...
NEGATIVE_SPACING_UTILITIES = frozenset({
    # Margin
    '-m-', '-mx-', '-my-', '-mt-', '-mr-', '-mb-', '-ml-', '-ms-', '-me-',
    # Translate
    '-translate-x-', '-translate-y-',
...
ROUNDED_MAPPING_PX: dict[int, str] = {
    2: 'xs', 4: 'sm', 6: 'md', 8: 'lg', 12: 'xl', 16: '2xl', 24: '3xl', 32: '4xl',
}
ROUNDED_MAPPING_REM: dict[float, str] = {
    0.125: 'xs', 0.25: 'sm', 0.375: 'md', 0.5: 'lg', 0.75: 'xl', 1.0: '2xl', 1.5: '3xl', 2.0: '4xl',
}
ROUNDED_DIRECTIONS = frozenset({'t', 'r', 'b', 'l', 'tl', 'tr', 'bl', 'br', 's', 'e', 'ss', 'se', 'es', 'ee'})

# border-[1px] → border (empty suffix)
BORDER_WIDTH_MAPPING: dict[int, str] = {0: '0', 1: '', 2: '2', 4: '4', 8: '8'}
BORDER_DIRECTIONS = frozenset({'t', 'r', 'b', 'l', 'x', 'y', 's', 'e', 'ss', 'se', 'es', 'ee'})

FONT_SIZE_MAPPING_PX: dict[int, str] = {
    12: 'xs', 14: 'sm', 16: 'base', 18: 'lg', 20: 'xl', 24: '2xl',
    30: '3xl', 36: '4xl', 48: '5xl', 60: '6xl', 72: '7xl', 96: '8xl', 128: '9xl',
}

FONT_SIZE_MAPPING_REM: dict[float, str] = {
...
BLUR_MAPPING_PX: dict[int, str] = {2: 'xs', 4: 'sm', 8: 'md', 12: 'lg', 16: 'xl', 24: '2xl', 40: '3xl'}

# Filter utilities that use percentage values
FILTER_UTILITIES = ('brightness', 'contrast', 'saturate', 'grayscale', 'sepia', 'invert')
BACKDROP_FILTER_UTILITIES = tuple(f'backdrop-{u}' for u in FILTER_UTILITIES) + ('backdrop-opacity',)

# Excluded directories for file search
...
def build_border_class(direction: str | None, suffix: str) -> str:
...
def find_border_shorthand_classes(content: str) -> list[Match]:
    ...
    for match in re.finditer(r'border-\[([\d.]+)px_([\d.]+)px(?:_([\d.]+)px)?(?:_([\d.]+)px)?]', content):
...
def _get_border_sides(values: list[int]) -> dict[str | None, int]:
...
def find_typography_classes(content: str) -> list[Match]:
    ...
    for match in re.finditer(r'text-\[(\d+(?:\.\d+)?)px]', content):
        ...
        if px_value_int in FONT_SIZE_MAPPING_PX:
